...
@since:     October 2012
@author:    Brian Kelly
@summary:   Set up peer nodes - copy of orignal script with edits for RHEL7
'''
...
class SetupLitpNodes():
    ...
    def setup_node_passwords(self):
        ...
        script = local_path + "/RHEL7_passwdsetup_litp_admin.exp"
        remote_path = "/tmp/RHEL7_passwdsetup_litp_admin.exp"
        ...
        for host in self.hostnames:
            ...
            for attempt in range(0, self.pw_attempts):
                ...
                cmd = "expect /tmp/RHEL7_passwdsetup_litp_admin.exp %s %s %s" %\
    ...
    def setup_litp_nodes(self):
        ...
        if not self.skip_passwd:
            ...
            cmd = "rm -rf /tmp/RHEL7_passwdsetup_litp_admin.exp /tmp/passwdsetup_root.exp /tmp/check_root_pw.exp /tmp/check_litp_admin_pw.exp /tmp/passwdsetup_root_noage.exp"
            stdout, stderr, retc = self.node.run_command(cmd)
            if retc != 0:
