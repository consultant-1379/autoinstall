...
from ilo_install.ilo_install_rhel6_10 import InsertIso, InstallIso
...
class AIRunner():
    def __init__(
            self, ai_option, rhel_iso, litp_iso, thirdpp, install_type,
            install_script, cluster_file, server_http_location,
            server_http_full_path_location, jenkins_job_id, install_option,
            ipmi_tool_location, os_patches, os_patches_path, litp_upgrade,
            litp_upgrade_iso, upgrade_3pp,
            ...
            expect_large_plan, pre_xml_script, local_trust_auth,
            monitor_script_path, skip_post_deploy_snapshot):
        """
        ...
        stdout, stderr, retc = self.utilfile.run_command_local("ip route get 1 | awk '{print $NF;exit}'", logs=False)
        ...
        self.ai_params["3PP_TARBALL"] = thirdpp
        ...
        self.ai_params["UPGRADE_3PP"] = upgrade_3pp
    ...
    def print_ai_info(self):
        ...
        print "3PP TAR.GZ FILE: ", self.ai_params["3PP_TARBALL"]
        ...
        if self.ai_params["UPGRADE_3PP"]:
            print "3PP OPTION CHOSEN"
    ...
    def create_lockfile(self):
        ...
        #if os.path.isfile(self.ai_params["LOCK_FILE"]):
        #    print ">> ERROR: %s is already being used by autoinstall" % self.ai_params["MS_IP"]
        #    sys.exit(1)
        # CREATE LOCK FILE SO NO ONE WILL AUTOINSTALL THE SAME NODE
        #cmd = "touch %s" % (self.ai_params["LOCK_FILE"])
        #stdout, stderr, retc = self.utilfile.run_command_local(cmd, logs=False)
        #if stdout != [] or stderr != [] or retc != 0:
        #    print ">> ERROR: CANNOT TOUCH FILE"
        #    sys.exit(1)
    ...
    def rhel_complete(self):
        ...
        self.install_litp()
        self.deploy_litp()
        if self.ai_params["INSTALL_TYPE"] == "cloud":
            try:
                InstallOSPatch(self.ai_params).install_os_patch_packages()
            except:
                print ">> WARNING: Install of OS Patch packages failed."
...
class AIUtil():
    ...
    def print_usage(self):
        ...
        + "python autoinstall.py [--complete-install|-ci] --rheliso= --litpiso= --thirdpp= --install_type= --install_script= --cluster_file= --server_http_location= --server_http_full_path_location= [OPTIONAL: --with_os_patches= --install_option= --pre_xml_load_script= --reboot_peer_nodes_after_install --skip_peer_node_passwd_setup --expect_large_plan]\n" \
        + "python autoinstall.py [--litp-ms-install|-litpms] --rheliso= --litpiso= --thirdpp= --install_type=  --cluster_file= --server_http_location= --server_http_full_path_location= [OPTIONAL: --with_os_patches=]\n" \
        ...
        + "python autoinstall.py [--rhel-ms-install|-rhelms] --litpiso= --thirdpp= --cluster_file= --server_http_full_path_location= [OPTIONAL: --with_os_patches= --ipmi_tool_location=]\n" \
        + "python autoinstall.py [--rhel-litp-install|-rhellitp] --litpiso= --thirdpp= --install_type= --install_script= --cluster_file= --server_http_full_path_location= [OPTIONAL: --with_os_patches= --install_option= --pre_xml_load_script= --ipmi_tool_location= --expect_large_plan]\n" \
        + "python autoinstall.py [--complete-litp-install-upgrade|-ciu] --rheliso= --litpiso= --thirdpp= --install_type= --install_script= --cluster_file= --server_http_location= --server_http_full_path_location= --litp_upgrade= [OPTIONAL: --with_os_patches= --install_option= --pre_xml_load_script= --restore-from-upgrade --with_os_patches_upgrade= --update-rsyslog8 --reboot_peer_nodes_after_install --skip_peer_node_passwd_setup --expect_large_plan]\n" \
...
def main():
    ...
    run = AIRunner(
        sys.argv[1], rhel_iso, litp_iso, thirdpp, install_type,
        install_script, cluster_file, server_http_location,
        server_http_full_path_location, jenkins_id, install_option,
        ipmi_tool_location, os_patches, os_patches_path, litp_upgrade,
        litp_upgrade_iso, upgrade_3pp, os_patches_upgrade,
