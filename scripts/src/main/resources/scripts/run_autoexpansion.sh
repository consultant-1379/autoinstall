...
if $RESTORE_EXPAND
then
    restore_expand="--restore-from-expand"
else
    restore_expand=""
fi

python autoinstall.py -elc --expand_script=$EXPAND_SCRIPT --cluster_file=$LITP_DATAREG --server_http_full_path_location=$AI_CLI_LOCATION $restore_expand
retval=( $(echo "$?") )
if [ $retval -ne 0 ]
then
    echo ""
