...
KS_FILE_STATIC = 'http://10.44.235.150/iso/litp/latest_RHEL7_ks/ms-ks-network.cfg'
...
class InstallIso():
    ...
    def __init__(self, ai_params):
        ...
        if self.ai_params["MS_BLADE_TYPE"] == "G8":
            self.timeout = {
                ...
                "install complete": 9600,
                "rebooting system": 1200,
                "ms1 login": 1200,
                "cmd respond": 15
            }
    ...
    def run_vsp_install(self):
        ...
        if self.ai_params["MS_BLADE_TYPE"] == "G8" or self.ai_params["MS_BLADE_TYPE"] == "G9":
            boot_cmd = "vmlinuz initrd=initrd.img inst.stage2=hd:LABEL=RHEL-7.9\\x20Server.x86_64 net.ifnames=0 biosdevname=0 console=ttyS0 ks=%s ip=%s::%s:%s:%s:eth0:none ksdevice=eth0 enforcing=0" % (self.ai_params["KS_FILE"], self.ai_params["MS_IP"], self.ai_params["MS_GATEWAY"], self.mask, self.ai_params["MS_HOSTNAME"])
        else:
            if len(self.ai_params["MS_VLAN"]) > 0:
                boot_cmd = "vmlinuz initrd=initrd.img inst.stage2=hd:LABEL=RHEL-7.9\\x20Server.x86_64 net.ifnames=0 biosdevname=0 bootdev=%s.%s ifname=%s:%s console=ttyS1 vlan=%s.%s:%s inst.ks=%s ip=%s::%s:%s:%s:%s.%s:none enforcing=0" % (self.ai_params["MS_INSTALL_NIC"], self.ai_params["MS_VLAN"], self.ai_params["MS_INSTALL_NIC"], self.ai_params["MS_INSTALL_MAC_ADDRESS"], self.ai_params["MS_INSTALL_NIC"], self.ai_params["MS_VLAN"], self.ai_params["MS_INSTALL_NIC"], self.ai_params["KS_FILE"], self.ai_params["MS_IP"], self.ai_params["MS_GATEWAY"], self.mask, self.ai_params["MS_HOSTNAME"], self.ai_params["MS_INSTALL_NIC"], self.ai_params["MS_VLAN"])
            else:
                boot_cmd = "vmlinuz initrd=initrd.img inst.stage2=hd:LABEL=RHEL-7.9\\x20Server.x86_64 net.ifnames=0 biosdevname=0 bootdev=%s ifname=%s:%s console=ttyS1 inst.ks=%s ip=%s::%s:%s:%s:%s:none enforcing=0" % (self.ai_params["MS_INSTALL_NIC"], self.ai_params["MS_INSTALL_NIC"], self.ai_params["MS_INSTALL_MAC_ADDRESS"], self.ai_params["KS_FILE"], self.ai_params["MS_IP"], self.ai_params["MS_GATEWAY"], self.mask, self.ai_params["MS_HOSTNAME"], self.ai_params["MS_INSTALL_NIC"])
        ...
        self.wait_for_output('Installation complete.  Press return to quit',
                             self.timeout["install complete"])
        self.send_key_press('\r', 'ENTER', 1)
        ...
        # REMOVE OS ISO FROM GATEWAY TO FREE UP SPACE
        print "Remove OS iso from gateway to free up space"
        cmd = 'rm -rf %s' % constants.RHEL_GW_LOCATION
