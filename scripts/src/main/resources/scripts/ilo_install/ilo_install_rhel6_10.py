...
# GLOBAL VARIABLES
KS_FILE_STATIC = 'http://10.44.235.150/tmp/rhel6_10_testing/ms-ks-network_RHEL6_10.cfg'
...
class InstallIso():
    ...
    def run_vsp_install(self):
        ...
        self.slow_send_vsp(boot_cmd, 0.1)
        ...
        if self.ai_params["MS_BLADE_TYPE"] == "G8" or self.ai_params["MS_BLADE_TYPE"] == "G9":
            ...
            self.slow_send_vsp(self.ai_params["MS_IP"], 0.1)
            self.send_key_press('\t', 'TAB', 1)
            print "Setting MS mask to %s" % self.mask
            self.slow_send_vsp(self.mask, 0.1)
            self.send_key_press('\t', 'TAB', 1)
            print "Setting MS gateway to %s" % self.ai_params["MS_GATEWAY"]
            self.slow_send_vsp(self.ai_params["MS_GATEWAY"], 0.1)
            self.send_key_press('\t', 'TAB', 1)
            self.send_key_press('\t', 'TAB', 1)
            self.send_key_press('\r', 'ENTER', 1)
        ...
        self.slow_send_vsp(self.ai_params["MS_HOSTNAME"], 0.1)
        self.send_key_press('\r', 'ENTER', 1)
