...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
 ...
 if  [ "${cluster[$i]}" !=  "3" ]; then
     ...
     if [ "${cluster[$i]}" =  "1" ]; then
        ...
        litp create -t bond -p /deployments/d1/clusters/c${cluster[$i]}/nodes/n$(($i+1))/network_interfaces/bond07 -o device_name=bond07 mode=1 ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6[$i]}" network_name=mgmt
        litp create -t vlan -p /deployments/d1/clusters/c${cluster[$i]}/nodes/n$(($i+1))/network_interfaces/vlan837 -o device_name=bond07.837 ipv6address="${node_ipv6_837[$i]}" network_name=net837v6
