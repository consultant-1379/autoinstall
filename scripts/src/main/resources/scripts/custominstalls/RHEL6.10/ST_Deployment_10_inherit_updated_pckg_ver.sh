...
#    litp inherit -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/routes/r5 -s /infrastructure/networking/routes/r1 -o subnet="${route_subnet_801}" gateway="${nodes_gateway_ext}"
   ...
   # litp inherit -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/routes/r8 -s /infrastructure/networking/routes/r1 -o subnet="${route836_subnet}" gateway="${node_ip[$i]}"
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/configs/alias_config/aliases/ntp_server_alias_4 -t alias -o alias_names="ntp4" address=10.44.86.4
...
# Add one VM 
litp create -t vm-image -p /software/images/image1 -o name=fmmed source_uri=http://${ms_host_short}/images/image_customscript.qcow2
...
litp create -t nameserver -p /ms/configs/dns_client/nameservers/my_name_server_1 -o ipaddress=10.44.86.14 position=1
...
litp create -t nameserver -p /deployments/d1/clusters/c1/nodes/n1/configs/dns_client/nameservers/my_name_server_1 -o ipaddress=10.44.86.14 position=1
...
litp create -t nameserver -p /deployments/d1/clusters/c1/nodes/n2/configs/dns_client/nameservers/my_name_server_1 -o ipaddress=10.44.86.14 position=1
