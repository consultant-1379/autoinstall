...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/file1 -t file-system -o type=ext4 mount_point=/file1 size=1G
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/file2 -t file-system -o type=ext4 mount_point=/file2 size=1G
...
litp create -t file-system -p /infrastructure/storage/storage_profiles/profile_2/volume_groups/vxvg1/file_systems/fs1 -o type=vxfs size=1G mount_point=/vxvm_vol1 snap_size=100 backup_snap_size=90
...
litp create -t nameserver -p /ms/configs/dns_client/nameservers/my_name_server_A -o ipaddress=10.44.86.14 position=1
...
litp create -t sfs-filesystem       -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl1/file_systems/mgmt_fs2 -o path="${sfs_prefix}-managed2" size='40M' snap_size=100 cache_name="${sfs_cache}"
...
litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/mount1_2 -o export_path="${sfs_prefix}-managed2" provider="virtserv1" mount_point="/SFS1_managed2" mount_options="soft,intr" network_name="netwrk837"


# FS managed on server 2
litp create -t sfs-filesystem       -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl2/file_systems/mgmt_fs1 -o path="${sfs_prefix}-p2-managed1" size='40M' snap_size=50 cache_name="${sfs_cache}"
litp create -t sfs-filesystem       -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl2/file_systems/mgmt_fs2 -o path="${sfs_prefix}-p2-managed2" size='40M' snap_size=100 cache_name="${sfs_cache}"

litp create -t sfs-export           -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl2/file_systems/mgmt_fs1/exports/ex1         -o ipv4allowed_clients="10.44.86.194,10.44.86.195,10.44.86.196" options="rw,no_root_squash"
litp create -t sfs-export           -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl2/file_systems/mgmt_fs2/exports/ex2         -o ipv4allowed_clients="10.44.86.194,10.44.86.195,10.44.86.196" options="rw,no_root_squash"

litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/mountp2_1 -o export_path="${sfs_prefix}-p2-managed1" provider="virtserv1" mount_point="/SFS1_P2_managed1" mount_options="soft,intr" network_name="netwrk837"
litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/mountp2_2 -o export_path="${sfs_prefix}-p2-managed2" provider="virtserv1" mount_point="/SFS1_P2_managed2" mount_options="soft,intr" network_name="netwrk837"
...
litp inherit -p /ms/file_systems/fs3 -s /infrastructure/storage/nfs_mounts/mountp2_1
litp inherit -p /deployments/d1/clusters/c1/nodes/n1/file_systems/fs3 -s /infrastructure/storage/nfs_mounts/mountp2_1
litp inherit -p /deployments/d1/clusters/c1/nodes/n1/file_systems/fs4 -s /infrastructure/storage/nfs_mounts/mountp2_2
litp inherit -p /deployments/d1/clusters/c1/nodes/n2/file_systems/fs3 -s /infrastructure/storage/nfs_mounts/mountp2_1
litp inherit -p /deployments/d1/clusters/c1/nodes/n2/file_systems/fs4 -s /infrastructure/storage/nfs_mounts/mountp2_2
...
litp create -t vm-ram-mount -p /software/services/vmservice1/vm_ram_mounts/fs_test_mount -o type=ramfs mount_point="/mnt/ram_test_mount" mount_options="size=64M,noexec,nosuid"
litp create -t vm-ram-mount -p /software/services/vmservice2/vm_ram_mounts/fs_test_mount -o type=ramfs mount_point="/mnt/ram_test_mount" mount_options="size=32M,noexec,nodev,nosuid"
...
litp create -t vm-ram-mount -p /software/services/vmservice4/vm_ram_mounts/fs_test_mount -o type=tmpfs mount_point="/mnt/tmp_test_mount" mount_options="size=128M,nosuid"
...
#litp update -p /deployments/d1/clusters/c1/services/SG_STvm1 -o initial_online_dependency_list=SG_STvm2,SG_STvm3
#litp update -p /deployments/d1/clusters/c1/services/SG_STvm3 -o initial_online_dependency_list=SG_STvm4
...
litp create -t vm-ram-mount -p /ms/services/msfmmed1/vm_ram_mounts/fs_test_mount -o type=tmpfs mount_point="/mnt/tmp_test_mount" mount_options="size=256M,nodev,nosuid"
# SFS FS mount
litp create -t vm-nfs-mount -p /ms/services/msfmmed1/vm_nfs_mounts/non_sfs_mount1 -o mount_point=/mnt/nfs_rw device_path="${nas_management_ip}:${nas_prefix}/rw_unmanaged" mount_options=defaults
# non-SFS FS mount
...
litp update -p /deployments/d1/clusters/c1/ -o cs_initial_online=off
...
# SAN Plugin 
##################################################################################
litp create -p /infrastructure/systems/san_sys2 -t blade -o system_name=san_sys2
litp create -t hba -p /infrastructure/systems/san_sys2/controllers/hba1 -o failover_mode=std hba_porta_wwn=50:01:43:80:26:e8:b8:7a 
litp create -t hba -p /infrastructure/systems/san_sys2/controllers/hba2 -o failover_mode=std hba_porta_wwn=50:01:43:80:26:e8:b8:78
litp create -t bmc -p  /infrastructure/systems/san_sys2/bmc -o ipaddress=10.44.84.133 password_key=key-for-root username=root
litp create -t lun-disk -p /infrastructure/systems/san_sys2/disks/disk0 -o balancing_group=low lun_name=LITP2_dot51_MN1_lun0 name=hd0 shared=false size=30G storage_container=SFS_Pool bootable=true snap_size=20     
litp create -t lun-disk -p /infrastructure/systems/san_sys2/disks/disk1 -o balancing_group=low lun_name=LITP2_dot51_MN1_lun1 name=hd1 shared=false size=2G storage_container=SFS_Pool
litp create -t lun-disk -p /infrastructure/systems/san_sys2/disks/disk2 -o balancing_group=low lun_name=LITP2_dot51_MN1_lun2 name=hd2 shared=false size=3G storage_container=SFS_Pool
litp create -t lun-disk -p /infrastructure/systems/san_sys2/disks/disk_vxvm_1 -o balancing_group=low lun_name=LITP2_dot51_vxvm_lun1 name=hdvx1 shared=true size=1G storage_container=SFS_Pool snap_size=20
litp create -t lun-disk -p /infrastructure/systems/san_sys2/disks/disk_vxvm_2 -o balancing_group=low lun_name=LITP2_dot51_vxvm_lun2 name=hdvx2 shared=true size=1G storage_container=SFS_Pool snap_size=20
litp create -t lun-disk -p /infrastructure/systems/san_sys2/disks/disk_vxvm_3 -o balancing_group=low lun_name=LITP2_dot51_vxvm_lun3 name=hdvx3 shared=true size=1G storage_container=SFS_Pool snap_size=20


litp create -p /infrastructure/systems/san_sys3 -t blade -o system_name=san_sys3
litp create -t hba -p /infrastructure/systems/san_sys3/controllers/hba1 -o failover_mode=std hba_porta_wwn=50:01:43:80:26:e8:b8:20 
litp create -t hba -p /infrastructure/systems/san_sys3/controllers/hba2 -o failover_mode=std hba_porta_wwn=50:01:43:80:26:e8:b8:22
litp create -t bmc -p  /infrastructure/systems/san_sys3/bmc -o ipaddress=10.44.84.134 password_key=key-for-root username=root
litp create -t lun-disk -p /infrastructure/systems/san_sys3/disks/disk0 -o balancing_group=low lun_name=LITP2_dot51_MN2_lun0 name=hd0 shared=false size=30G storage_container=SFS_Pool bootable=true snap_size=20
litp create -t lun-disk -p /infrastructure/systems/san_sys3/disks/disk1 -o balancing_group=low lun_name=LITP2_dot51_MN2_lun1 name=hd1 shared=false size=2G storage_container=SFS_Pool
litp create -t lun-disk -p /infrastructure/systems/san_sys3/disks/disk2 -o balancing_group=low lun_name=LITP2_dot51_MN2_lun2 name=hd2 shared=false size=3G storage_container=SFS_Pool
litp create -t lun-disk -p /infrastructure/systems/san_sys3/disks/disk_vxvm_1 -o balancing_group=low lun_name=LITP2_dot51_vxvm_lun1 name=hdvx1 shared=true size=1G storage_container=SFS_Pool snap_size=20
litp create -t lun-disk -p /infrastructure/systems/san_sys3/disks/disk_vxvm_2 -o balancing_group=low lun_name=LITP2_dot51_vxvm_lun2 name=hdvx2 shared=true size=1G storage_container=SFS_Pool snap_size=20
litp create -t lun-disk -p /infrastructure/systems/san_sys3/disks/disk_vxvm_3 -o balancing_group=low lun_name=LITP2_dot51_vxvm_lun3 name=hdvx3 shared=true size=1G storage_container=SFS_Pool snap_size=20

litp remove -p /deployments/d1/clusters/c1/nodes/n1/system
litp remove -p /deployments/d1/clusters/c1/nodes/n2/system
litp inherit -p /deployments/d1/clusters/c1/nodes/n2/system -s /infrastructure/systems/san_sys3
litp inherit -p /deployments/d1/clusters/c1/nodes/n1/system -s /infrastructure/systems/san_sys2

litpcrypt set key-for-san-ie root shroot12
litp remove -p /infrastructure/systems/sys2/
litp remove -p /infrastructure/systems/sys3

litp create -t san-emc -p /infrastructure/storage/storage_providers/emc_vnx -o ip_a=10.44.84.144 ip_b=10.44.84.145 name=Atvnx-43 password_key=key-for-san-ie storage_network=data username=root login_scope=local san_type=vnx1 storage_site_id=st_storage
litp create -t storage-container -p /infrastructure/storage/storage_providers/emc_vnx/storage_containers/pool1 -o name=SFS_Pool type=POOL
litp create -t storage-container -p /infrastructure/storage/storage_providers/emc_vnx/storage_containers/rg1 -o name=0 type=RAID_GROUP
# Remove physical_devices as there appears to be an issue with san_plugin adding more than on LVM disk 
litp remove -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/physical_devices/pd1 
litp remove -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/physical_devices/pd2
