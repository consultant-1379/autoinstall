...
# SFS Filesystems and Shares - node1 only
...
#litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/mount1 -o export_path="${sfs_prefix}-fs1" provider="virtserv1" mount_point="/cluster1" mount_options="soft,intr" network_name="mgmt"
...
#litp inherit -p  /infrastructure/storage/file_systems/fs1/export -s /infrastructure/storage/storage_providers/nfs_service/exports/ex1
#litp inherit -p  /infrastructure/storage/file_systems/fs1/vip -s /infrastructure/storage/storage_providers/nfs_service/ip_addresses/vip
...
# Service Groups
# 2 F/O SGs - 1st SG #VIP=2x#AC 2nd SG  #VIP=2x#AC
# 2 PL  SGs - 3rd SG #VIP=2x#AC 4th SG  #VIP=2x#AC
x=0
SG_pkg[x]="cups";       SG_VIP_count[x]=2;      SG_active[x]=1; SG_standby[x]=1 status_interval[x]=10	status_timeout[x]=3600	restart_limit[x]=0	startup_retry_limit[x]=99999 version[x]=1.4.2 release[x]=48.el6_3.3	x=$[$x+1]
