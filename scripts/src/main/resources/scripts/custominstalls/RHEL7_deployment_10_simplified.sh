...
#    No VM on MS
#    No file systems on MS
#    Uses eth4 and eth5 on MS
#    No alternative to RHEL6 luci or ricci services

#
# Usage:
#   RHEL7_deploy_multiblade_san_SP21_07_simplified.sh <CLUSTER_SPEC_FILE>
#
...
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs6          -o type='xfs' mount_point='/var/www'  size='70G' snap_size='15'
...
# NFS MOUNTS - Commented out for now
...
litp create -p /ms/network_interfaces/if4 -t eth -o device_name=eth4 macaddress="${ms_eth4_mac}" ipaddress="${ms_ip}" ipv6address="${ms_ipv6_00}" network_name=mgmt

# RE_WORK THE NETWORK SO ETH0 IS BRIDGED
litp update -p /ms/network_interfaces/if4 -d ipaddress ipv6address network_name
litp update -p /ms/network_interfaces/if4 -o bridge=br4
litp create -p /ms/network_interfaces/br4 -t bridge -o ipaddress="${ms_ip}" ipv6address="${ms_ipv6_00}" device_name=br4 network_name=mgmt
...
# NFS MOUNTS FOR MS - commented out for now
...
litp import /tmp/test_services/ci_test_service1-1.0-1.noarch.rpm 3pp_rhel7
...
# MS BRIDGING
litp create -p /ms/network_interfaces/if5 -t eth -o device_name=eth5 macaddress="${ms_eth5_mac}"
litp create -p /ms/network_interfaces/br5 -t bridge -o device_name=br5 network_name=net1vm ipaddress="${net1vm_ip_ms}"
litp create -p /ms/network_interfaces/vlan911 -t vlan -o device_name=eth5.911 bridge=br5
