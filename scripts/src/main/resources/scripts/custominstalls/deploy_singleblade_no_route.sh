...
litp create -p /infrastructure/networking/ip_ranges/range1 -t ip-range -o network_name="nodes" start="$nodes_ip_start" end="$nodes_ip_end" subnet="$nodes_subnet" gateway="$nodes_gateway"
litp create -p /infrastructure/networking/ip_ranges/range2 -t ip-range -o network_name="ms_external" start="$ms_ip" end="$ms_ip" subnet="$ms_subnet" gateway="$ms_gateway"
...
litp create -p /infrastructure/networking/network_profiles/libvirt_provider/networks/network1 -t network -o network_name="ms_external" interface="if0" default_gateway="true"
...
for (( i=0; i<${#node_hostname[@]}; i++ )); do
    ...
    litp link -p /deployments/d1/clusters/cluster1/nodes/node$(($i+1))/items/ntp1 -t ntp-service
done
