...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /infrastructure/systems/sys$(($i+2))/disks/disk0 -t disk -o name=hd0 size=28G bootable=true uuid="${node_disk0_uuid[$i]}"
...
litp create -t eth -p /ms/network_interfaces/if2 -o device_name=eth2 macaddress="${ms_eth2_mac}" ipv6address="${ms_ipv6_834}"  network_name=834 
