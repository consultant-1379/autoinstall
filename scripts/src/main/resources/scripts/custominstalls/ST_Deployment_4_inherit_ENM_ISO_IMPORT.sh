...
litp create  -t disk                -p /infrastructure/systems/sys1/disks/d1 -o name="hd1" size=22G bootable="true" uuid=$ms_disk_uuid
...
#for (( i=0; i<7; i++ )); do
litp create -p /infrastructure/storage/storage_profiles/profile_2/volume_groups/vg_vxvm_0/file_systems/VxVM_VG2_1 -t file-system     -o type=vxfs mount_point=/VxVM_mp_VG2_FS1 size=50M snap_size=30
#done

# Unmounted FS1 for LVM volume group vg1
litp create -t file-system -p  /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/unmounted_fs1 -o type=ext4 size=52M snap_size=30


# Unmounted FS2 for LVM volume group vg1
litp create -t file-system -p  /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/unmounted_fs2 -o type=ext4 size=52M snap_size=30
...
litp create -p /infrastructure/storage/storage_profiles/profile_5/volume_groups/vg_vxvm_0_snap/file_systems/No_snap            -t file-system     -o type=vxfs mount_point=/no_snap size=30M snap_size=0 snap_external=true
...
#bridge mgmt bond
    litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n1/network_interfaces/br0            -o device_name=br0  network_name=mgmt      ipaddress="${node_ip[0]}"      ipv6address="${ipv6_00[0]}" multicast_snooping=0 
...
# Network Node 2
  ...
  litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n2/network_interfaces/br0          -o device_name=br0 network_name=mgmt      ipaddress="${node_ip[1]}"      ipv6address="${ipv6_00[1]}" 
...
litp create -t sfs-export           -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl1/file_systems/mgmt_fs1/exports/ex1         -o ipv4allowed_clients="${ms_ip_ext2},${node_ip_ext2[0]},${node_ip_ext2[1]}" options="ro,no_root_squash,secure_locks"
litp create -t sfs-export           -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl2/file_systems/mgmt_fs3/exports/ex1         -o ipv4allowed_clients="${ms_ip_ext2},${node_ip_ext2[0]},${node_ip_ext2[1]}" options="rw,no_root_squash,no_subtree_check"
litp create -t sfs-export           -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl1/file_systems/mgmt_fs2/exports/ex1         -o ipv4allowed_clients="${ms_ip_ext2},${node_ip_ext2[0]},${node_ip_ext2[1]}" options="ro,no_root_squash,secure_locks"
...
litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/mgmt_sfs01 -o export_path="${sfs_prefix}_mgmt_sfs_fs1"   provider="virtserv1" mount_point="/mgmt_sfs_fs01" mount_options="soft"      network_name="netwk837"
litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/mgmt_sfs02 -o export_path="${sfs_prefix}_mgmt_sfs_fs2"   provider="virtserv1" mount_point="/mgmt_sfs_fs02" mount_options="soft"      network_name="netwk837"
...
litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/mgmt_sfs03 -o export_path="${sfs_prefix}_mgmt_sfs_fs3"   provider="virtserv1" mount_point="/mgmt_sfs_fs03" mount_options="soft"      network_name="netwk837"
...
litp create -t nameserver -p /ms/configs/dns_client/nameservers/my_name_server_A -o ipaddress="${ipv6_834[0]}" position=1
...
# lsb service
x=0
pkg[x]="luci";       rel[x]="63.el6";     ver[x]="0.26.0";     VIP_count[x]=4;      active[x]=1; standby[x]=1 status_interval[x]=10   status_timeout[x]=10    restart_limit[x]=0      startup_retry_limit[x]=0     node_list[x]="n1,n2" initial_online_dependency_list[x]="test_service,httpd,ricci,cups" x=$[$x+1]
pkg[x]="httpd";      rel[x]="39.el6";     ver[x]="2.2.15";     VIP_count[x]=$[5*2]; active[x]=2; standby[x]=0 status_interval[x]=20   status_timeout[x]=30    restart_limit[x]=30     startup_retry_limit[x]=40    node_list[x]="n2,n1" initial_online_dependency_list[x]="cups" x=$[$x+1]
...
pkg[x]="cups";       rel[x]="67.el6";     ver[x]="1.4.2";      VIP_count[x]=3;      active[x]=1; standby[x]=1 status_interval[x]=40   status_timeout[x]=30    restart_limit[x]=3      startup_retry_limit[x]=2     node_list[x]="n2,n1" initial_online_dependency_list[x]="ricci" x=$[$x+1]
pkg[x]="ricci";      rel[x]="75.el6";     ver[x]="0.16.2";     VIP_count[x]=$[2*2]; active[x]=2; standby[x]=0 status_interval[x]=1000 status_timeout[x]=1000  restart_limit[x]=1000   startup_retry_limit[x]=1000  node_list[x]="n1,n2" x=$[$x+1]
...
litp inherit -p /deployments/d1/clusters/c1/services/cups/filesystems/fs2     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_2/file_systems/VxVM_VG3_2 # -o snap_size=10
litp inherit -p /deployments/d1/clusters/c1/services/luci/filesystems/fs3     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_3/file_systems/VxVM_VG3_3 # -o snap_size=20
litp inherit -p /deployments/d1/clusters/c1/services/luci/filesystems/fs4     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_4/file_systems/VxVM_VG3_4 # -o snap_size=100
litp inherit -p /deployments/d1/clusters/c1/services/luci/filesystems/fs5     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_5/file_systems/VxVM_VG3_5 # -o snap_size=70
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp inherit -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/items/libguestfs-tools -s /software/items/libguestfs-tools
 #LITPCDS-11129
...
#litp update -p /software/items/test_service -o epoch=1
...
for (( i=1; i<=${#VM_cpu[@]}; i++ )); do
    ...
    litp create -t vm-yum-repo -p /software/services/se_vm$i/vm_yum_repos/updates -o name=vm_UPDATES base_url="http://dot66-ms/6/updates/x86_64/Packages"
    litp create -t vm-yum-repo -p /software/services/se_vm$i/vm_yum_repos/os -o name=vm_os base_url=http://10.44.86.66/6/os/x86_64
    litp create -t vm-alias -p /software/services/se_vm$i/vm_aliases/ms_alias -o alias_names=dot66-ms,bossman address=10.44.86.66
    litp create -t vm-alias -p /software/services/se_vm$i/vm_aliases/mn1_alias -o alias_names=node1,underling1 address=10.44.86.67
    litp create -t vm-alias -p /software/services/se_vm$i/vm_aliases/mn2_alias -o alias_names=node2,underling2 address=10.44.86.68
