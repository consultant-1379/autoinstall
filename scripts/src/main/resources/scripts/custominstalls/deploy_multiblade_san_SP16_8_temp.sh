...
if [ "$exclude_vxvm" != "exclude" ]
then
    litp create -p /deployments/d1/clusters/c1 -t vcs-cluster -o cluster_type=sfha low_prio_net=traffic2 llt_nets='hb1,hb2' cluster_id="${cluster_id}" default_nic_monitor="mii" critical_service="$FO_SG_pkg"
else
...
litp create -p /deployments/d1/clusters/c1/services/"$FO_SG_pkg" -t vcs-clustered-service -o active=1 standby=1 name=FO_vcs1 online_timeout=45 node_list='n2,n1' dependency_list=$PL_SG_pkg,FO_SG_vm2
...
litp create -p /deployments/d1/clusters/c1/services/"$PL_SG_pkg" -t vcs-clustered-service -o active=2 standby=0 name=PL_vcs node_list='n1,n2' dependency_list=FO_SG_vm2
...
litp create -p /software/images/image1 -t vm-image -o name="FO_SG_vm2" source_uri="http://${ms_ip}/images/vm_rhel_7_test_image-1-1.0.1.qcow2"

litp create -p /software/services/vmservice1 -t vm-service -o service_name="CIvmserv1" image_name="FO_SG_vm2" cpus=2 ram=2000M internal_status_check=on cleanup_command="/sbin/service CIvmserv1 force-stop-undefine"
litp create -p /deployments/d1/clusters/c1/services/FO_SG_vm2 -t vcs-clustered-service -o name="FO_SG_vm2" active=1 standby=1 node_list='n1,n2' online_timeout=300

litp inherit -p /deployments/d1/clusters/c1/services/FO_SG_vm2/applications/vmservice1 -s /software/services/vmservice1
...
litp update -p /deployments/d1/clusters/c1/services/FO_SG_vm2/applications/vmservice1/vm_network_interfaces/vm_nic1 -o ipaddresses="${vm_ip[0]}" gateway=${net1vm_gateway} ipv6addresses="${vm_ip6[0]}" gateway6=${net1vm_gateway6}
