...
litp create -p /ms/network_interfaces/if2 -t eth -o device_name=eth2 macaddress="${ms_eth2_mac}" master=bondData
litp create -p /ms/network_interfaces/if3 -t eth -o device_name=eth3 macaddress="${ms_eth3_mac}" master=bondData

litp create -p /ms/network_interfaces/b1 -t bond -o device_name=bondData ipaddress="${ms_ip_2}" network_name=traffic3
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/bondMgmt -t bond -o device_name='bondMgmt' ipaddress="${node_ip[$i]}" network_name=mgmt
