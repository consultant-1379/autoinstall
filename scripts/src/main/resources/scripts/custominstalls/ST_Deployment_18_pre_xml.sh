...
#litp create -t vcs-cluster -p /deployments/d1/clusters/c1 -o cluster_type=vcs low_prio_net=net898 llt_nets=hb1,hb2 app_agent_num_threads=30  cluster_id="${vcs_cluster_id}" cs_initial_online=on default_nic_monitor=mii
...
#l#itp inherit -p /deployments/d1/clusters/c1/services/apache/applications/testservice2 -s /software/services/testservice2
#l#itp create -t ha-service-config -p /deployments/d1/clusters/c1/services/ricci/ha_configs/conf_ricci -o status_interval=15 status_timeout=50 restart_limit=7
#l#itp create -t ha-service-config -p /deployments/d1/clusters/c1/services/apache/ha_configs/conf_httpd1 -o status_interval=30 status_timeout=59 restart_limit=2 startup_retry_limit=3 tolerance_limit=2 clean_timeout=58 service_id=httpd_service dependency_list=testservice1,testservice2
