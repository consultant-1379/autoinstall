...
#
# Usage:
#   ST_Deployment_10_inherit_test.sh <CLUSTER_SPEC_FILE>
#
#
...
litp create -t network -p /infrastructure/networking/networks/ipv61 -o name=ipv62
...
litp create -t bond -p /ms/network_interfaces/b0 -o device_name='bond0' mode=1 miimon=100 ipaddress="${ms_ip}" ipv6address="${ms_ipv6}" network_name=mgmt
litp create -t bond -p /ms/network_interfaces/b1 -o device_name='bond1' ipaddress="${ms_ip_ext}" network_name=data mode=1 miimon=100 ipaddress="${ms_ip}" ipv6address="${ms_ipv6}" network_name=mgmt

litp create -t vlan -p /ms/network_interfaces/bond0_898 -o device_name='bond0.898' ipaddress="${ms_ip_backup}" ipv6address="${ms_ipv6_backup}" network_name='backup'
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
  ...
  litp create -t bond -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/b0 -o device_name='bond0' ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6[$i]}" network_name=mgmt mode=1 miimon=100

  litp create -t vlan -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/bond0_898  -o device_name='bond0.898' ipaddress="${node_ip_898[$i]}" ipv6address="${node_ipv6_898[$i]}" network_name='backup'
  ...
  litp create -t vlan -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/bond0_834  -o device_name='bond0.834' ipv6address="${node_ipv6_834[$i]}" network_name=ipv61 
 ...
 #   litp inherit -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/routes/r3 -s /infrastructure/networking/routes/r1 -o subnet="${route3_subnet}" gateway="${nodes_gateway}"
    ...
    # route for bond.898 (LITPCDS-6077)
    litp inherit -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/routes/r6 -s /infrastructure/networking/routes/r1 -o subnet="${route5_subnet}" gateway="${node_ip_898[$i]}"
    # route for bond.836 (LITPCDS-6077)
    litp inherit -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/routes/r7 -s /infrastructure/networking/routes/r1 -o subnet="${nodes_subnet_ext}" gateway="${node_ip_836[$i]}"
    # route for bond0
...
# SFS Filesystems and Shares - node1 and MS
...
litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/sfs1 -o export_path="${sfs_prefix}-fs1" provider="virtserv1" mount_point="/cluster1" mount_options="soft,intr" network_name="mgmt"

litp inherit -p /deployments/d1/clusters/c1/nodes/n1/file_systems/sfs1 -s /infrastructure/storage/nfs_mounts/sfs1
#litp inherit -p /ms/file_systems/sfs1 -s /infrastructure/storage/nfs_mounts/sfs1
...
litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/nfs1 -o export_path="${nfs_prefix}/ro_unmanaged" provider="nfs1" mount_point="/cluster_ro" mount_options="soft,intr,timeo=30,noexec,nosuid" network_name="mgmt"
litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/nfs2 -o export_path="${nfs_prefix}/rw_unmanaged" provider="nfs1" mount_point="/cluster_rw" mount_options="soft,intr,timeo=30,noexec,nosuid" network_name="mgmt"
...
#litp inherit -p /ms/file_systems/nfs1 -s /infrastructure/storage/nfs_mounts/nm1
#litp inherit -p /ms/file_systems/nfs2 -s /infrastructure/storage/nfs_mounts/nm2
...
ip_count=1
litp create -t package               -p /software/items/"${SG_pkg[$x]}" -o name="${SG_pkg[$x]}" release="${release[$x]}" version="${version[$x]}"
...
# Mount a VxVM Volume on dovecot (F/O)
litp inherit -p /deployments/d1/clusters/c1/services/SG_dovecot/runtimes/dovecot/filesystems/fs1 -s /deployments/d1/clusters/c1/storage_profile/sp2/volume_groups/vg_vmvx/file_systems/VG2_FS_1
