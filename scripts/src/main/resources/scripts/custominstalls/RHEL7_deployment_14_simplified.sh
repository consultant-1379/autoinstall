#!/bin/bash
#
# Based on RHEL6 deploy_multibalde_san_SP21_07.sh
#  simplified:
#    no traffic3 network
#
# Usage:
#   RHEL7_deployment_14_simplified.sh <CLUSTER_SPEC_FILE>
#
...
litp create -p /software/items/dovecot -t package -o name=dovecot release=8.el7 version=2.2.36 epoch=1
...
litp import /tmp/test_services/3PP-dutch-hello-1.0.0-1.noarch.rpm 3pp_rhel7
litp import /tmp/test_services/3PP-english-hello-1.0.0-1.noarch.rpm 3pp_rhel7
...
# MS STORAGE PROFILE
litp create -t storage-profile -p /infrastructure/storage/storage_profiles/profile_ms
litp create -t volume-group    -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1                           -o volume_group_name='vg_root'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/dataA        -o type='xfs' mount_point='/nested/nested/dataA'    size=400M
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/data_umount  -o type='xfs'                         size='48M'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs1          -o type='xfs' mount_point='/'         size='70G' snap_size='15'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs2          -o type='swap' mount_point='swap'      size='2G'  snap_size='15'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs3          -o type='xfs' mount_point='/home'     size='12G'  snap_size='15'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs4          -o type='xfs' mount_point='/var'      size='35G' snap_size='15'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs5          -o type='xfs' mount_point='/var/log'  size='20G' snap_size='15'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs6          -o type='xfs' mount_point='/var/www'  size='140G' snap_size='15'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs7          -o type='xfs' mount_point='/software' size='300G' snap_size='15'
litp create -t physical-device -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/physical_devices/internal -o device_name='hd0'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs8          -o type='xfs' mount_point='/var/lib/mysql'  size='25G' snap_size='10'
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs9          -o type='xfs' mount_point='/var/lib/libvirt' size='20G' snap_size='10'
...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/data1 -t file-system -o type=xfs mount_point=/data1 size=2G snap_size=0
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/data2_umount -t file-system -o type=xfs  size=500M
...
litp create -p /infrastructure/systems/sys1/disks/disk0 -t disk -o name=hd0 size=1200G bootable=true uuid="${ms_disk_uuid}"
...
litp create -p /ms/configs/fw_config_init/rules/fw_dns -t firewall-rule -o action=accept dport=53 'name=021 DNS udp' proto=udp state=none
...
litp create -t firewall-rule -p /deployments/d1/clusters/c1/configs/fw_config_init/rules/fw_dnsudp -o name="071 dns" dport=53 proto="udp" state=none
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}" network_name='mgmt' multicast_snooping=0
...
litp create -p /deployments/d1/clusters/c1/services/"$FO_SG_pkg"/ha_configs/conf1 -t ha-service-config -o status_interval=50 status_timeout=50 restart_limit=10 startup_retry_limit=3 service_id=cups 
...
litp create -p /software/images/image1 -t vm-image -o name="PL_SG_vm1" source_uri="http://${ms_ip}/images/vm_image_rhel6.qcow2"
litp create -p /software/services/vmservice1 -t vm-service -o service_name="CIvmserv1" image_name="PL_SG_vm1" cpus=4 ram=2000M internal_status_check=on cleanup_command="/usr/bin/systemctl stop CIvmserv1"
...
litp create -p /software/services/vmservice1/vm_yum_repos/os -t vm-yum-repo -o name=os base_url="http://${ms_ip}/7/os/x86_64"
litp create -p /software/services/vmservice1/vm_yum_repos/updates -t vm-yum-repo -o name=rhelPatches base_url="http://${net1vm_ip_ms}/7/updates/x86_64/Packages" # UPDATE
...
litp create -p /software/images/image2 -t vm-image -o name="FO_SG_vm1" source_uri="http://${ms_ip}/images/vm_image_rhel7.qcow2"
litp create -p /software/services/vmservice2 -t vm-service -o service_name="CIvmserv2" image_name="FO_SG_vm1" cpus=2 ram=4000M internal_status_check=on cleanup_command="/usr/bin/systemctl stop CIvmserv2"
...
litp create -p /software/services/vmservice2/vm_yum_repos/os -t vm-yum-repo -o name=os base_url="http://${ms_ip}/7/os/x86_64"
litp create -p /software/services/vmservice2/vm_yum_repos/updates -t vm-yum-repo -o name=rhelPatches base_url="http://${net1vm_ip_ms}/7/updates/x86_64/Packages" # UPDATE
...
########## MS VM SERVICE #############
litp create -p /software/images/ms_vm_image1 -t vm-image -o name="MS_vm1" source_uri="http://${ms_ip}/images/vm_image_rhel6.qcow2"
...
then
    ...
    litp create -p /ms/services/ms_vmservice1/vm_yum_repos/os -t vm-yum-repo -o name=os base_url="http://${ms_ip}/7/os/x86_64"
    litp create -p /ms/services/ms_vmservice1/vm_yum_repos/updates -t vm-yum-repo -o name=rhelPatches base_url="http://${net1vm_ip_ms}/7/updates/x86_64/Packages"
else
    ...
    litp create -p /ms/services/ms_vmservice1/vm_yum_repos/os -t vm-yum-repo -o name=os base_url="http://${ms_ip}/7/os/x86_64"
    litp create -p /ms/services/ms_vmservice1/vm_yum_repos/updates -t vm-yum-repo -o name=rhelPatches base_url="http://${ms_ip}/7/updates/x86_64/Packages"
