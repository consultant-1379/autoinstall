...
# Interfaces
litp create -t eth  -p /ms/network_interfaces/if0       -o device_name=eth0 macaddress="${ms_eth0_mac}" rx_ring_buffer=2039 tx_ring_buffer=2039 txqueuelen=1250
...
# Network Node 1
    ...
    litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n1/network_interfaces/mybr1       -o device_name=br834   forwarding_delay=0            network_name=net834    ipaddress="${net834_ip[0]}"  ipv6address="${ipv6_01[0]}" multicast_snooping=1 hash_max=512 multicast_router=2 multicast_querier=1
    litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n1/network_interfaces/mybr2       -o device_name=br837   forwarding_delay=0            network_name=net837    ipaddress="${net837_ip[0]}" multicast_snooping=1 hash_max=1024 multicast_router=1 multicast_querier=0
    litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n1/network_interfaces/mybr3       -o device_name=br_vip1 forwarding_delay=0            network_name=net1vm    ipaddress="${VM_net1vm_ip[0]}" multicast_snooping=1 hash_max=256 multicast_router=0 multicast_querier=1
    ...
    litp create -t  eth -p /deployments/d1/clusters/c1/nodes/n1/network_interfaces/if2           -o device_name=eth2 macaddress="${node_eth2_mac[0]}" network_name=heartbeat1 rx_ring_buffer=2039 tx_ring_buffer=2039 txqueuelen=1250
...
litp create -t alias -p /deployments/d1/clusters/c1/nodes/n2/configs/alias_config/aliases/initial_alias -o alias_names="original-alias,secondary-alias" address="127.0.0.1"

litp create -p /deployments/d1/clusters/c1/nodes/n1/configs/alias_config -t alias-node-config
litp create -p /deployments/d1/clusters/c1/nodes/n1/configs/alias_config/aliases/initial_alias -t alias -o alias_names="original-alias,secondary-alias" address="127.0.0.1"
...
for (( i=0; i<10; i++)); do
    ...
    litp create -t sfs-export           -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl1/file_systems/mgmt_fs$i/exports/ex1 -o ipv4allowed_clients="$allowed_ips" options="rw,no_root_squash,secure_locks"
    litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/mgmt_sfs$i -o provider="virtserv1" mount_point="/mgmt_sfs_fs"$i mount_options="soft" network_name=${nas_network} export_path="${sfs_prefix}_mgmt_sfs-fs"$i
...
/usr/bin/md5sum /var/www/html/images/image_customscript.qcow2 | cut -d ' ' -f 1 > /var/www/html/images/image_customscript.qcow2.md5
...
litp import /tmp/EXTR-lsbwrapper39-2.0.0.rpm /var/www/html/3pp
litp import /tmp/EXTR-lsbwrapper40-2.0.0.rpm /var/www/html/3pp
...
litp create -t package -p /software/items/EXTR_lsbwrapper39 -o name='EXTR-lsbwrapper39' version=2.0.0-1 
litp create -t package -p /software/items/EXTR_lsbwrapper40 -o name='EXTR-lsbwrapper40' version=2.0.0-1
...
SG_pkg[x]="EXTR_lsbwrapper39";      SG_VIP_count[x]=$[0*2]; SG_active[x]=1; SG_standby[x]=0 status_interval[x]=100 status_timeout[x]=100  restart_limit[x]=10  startup_retry_limit[x]=10  node_list[x]="n1" offline[x]="40"  x=$[$x+1]
...
litp update -p /software/services/EXTR_lsbwrapper39 -o cleanup_command='/bin/touch /tmp/test-lsb-39.cleanup' service_name='test-lsb-39' stop_command='/sbin/service test-lsb-39 stop' status_command='/sbin/service test-lsb-39 status' start_command='/sbin/service test-lsb-39 start'  
