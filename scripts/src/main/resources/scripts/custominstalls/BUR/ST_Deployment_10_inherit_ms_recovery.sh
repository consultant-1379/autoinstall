...
# DISKS
litp create -t disk -p /infrastructure/systems/sys1/disks/d1 -o name="hdms1" size="500G" bootable="true" uuid="${ms1_disk_uuid}"
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /infrastructure/systems/sys$(($i+2))/disks/disk1 -t disk -o name=hd1a size=28G bootable=false uuid="${vxvm1_disk_uuid}"
...
litp create -t eth -p /ms/network_interfaces/if0 -o device_name=eth0 macaddress="${ms_eth0_mac}" ipaddress="${ms_ip}" ipv6address="${ms_ipv6_835}" network_name=mgmt 
...
litp create -t alias -p /ms/configs/alias_config/aliases/ntp_alias_1 -o alias_names=ntp-alias-1 address=10.44.86.30
litp create -t alias -p /ms/configs/alias_config/aliases/ntp_alias_2 -o alias_names=ntp-alias-2 address=127.127.1.0
litp create -t alias -p /ms/configs/alias_config/aliases/ntp_alias_3 -o alias_names=ntp-alias-3 address=10.44.86.66
