#!/bin/bash
#
# Based on RHEL6 deployment_multiblade_san_va.sh
#
# Usage:
#   RHEL7_deployment_100_simplified.sh <CLUSTER_SPEC_FILE>
#
...
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs1          -o type='xfs' mount_point='/'         size='70G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs2          -o type='swap' mount_point='swap'      size='2G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs3          -o type='xfs' mount_point='/home'     size='12G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs4          -o type='xfs' mount_point='/var'      size='35G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs5          -o type='xfs' mount_point='/var/log'  size='20G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs6          -o type='xfs' mount_point='/var/www'  size='140G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs7          -o type='xfs' mount_point='/software' size='300G' snap_size=0
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs8          -o type='xfs' mount_point='/var/lib/mysql'  size='25G' snap_size=50
litp create -t file-system     -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/fs9          -o type='xfs' mount_point='/var/lib/libvirt' size='20G' snap_size=50
...
litp create -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/sfs_pool1/file_systems/managed_fs2/exports/export2 -t sfs-export -o ipv4allowed_clients="${ms_subnet}" options="ro,root_squash"
...
litp create -p /ms/network_interfaces/if1 -t eth -o device_name=eth1 macaddress="${ms_eth1_mac}" master=bondmgmt rx_ring_buffer=0 tx_ring_buffer=0
...
litp create -p /deployments/d1/clusters/c1 -t vcs-cluster -o cluster_type=sfha low_prio_net=traffic2 llt_nets='hb1,hb2' cluster_id="${cluster_id}" default_nic_monitor="mii" vcs_seed_threshold=1
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_eth0_mac[$i]}" ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}" network_name='mgmt' txqueuelen=750

    # HEARTBEAT NETWORK SETUP
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if2 -t eth -o device_name=eth2 macaddress="${node_eth2_mac[$i]}" network_name=hb1 rx_ring_buffer=453 tx_ring_buffer=4078
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if6 -t eth -o device_name=eth6 macaddress="${node_eth6_mac[$i]}" network_name='traffic3' ipaddress="${node_ip_4[$i]}" rx_ring_buffer=2039 tx_ring_buffer=2039
...
# SETUP THE VM_IMAGE
/usr/bin/md5sum /var/www/html/images/vm_test_image-2-1.0.4.qcow2 | cut -d ' ' -f 1 | tee /var/www/html/images/vm_test_image-2-1.0.4.qcow2.md5 2>&1
/usr/bin/md5sum /var/www/html/images/vm_test_image_neg-1-1.0.4.qcow2 | cut -d ' ' -f 1 | tee /var/www/html/images/vm_test_image_neg-1-1.0.4.qcow2.md5 2>&1
/usr/bin/md5sum /var/www/html/images/vm_test_image_neg-2-1.0.4.qcow2 | cut -d ' ' -f 1 | tee /var/www/html/images/vm_test_image_neg-2-1.0.4.qcow2.md5 2>&1
/usr/bin/md5sum /var/www/html/images/vm_test_image-5-1.0.4.qcow2 | cut -d ' ' -f 1 | tee /var/www/html/images/vm_test_image-5-1.0.4.qcow2.md5 2>&1
/usr/bin/md5sum /var/www/html/images/vm_test_image_neg-3-1.0.3.qcow2 | cut -d ' ' -f 1 | tee /var/www/html/images/vm_test_image_neg-3-1.0.3.qcow2.md5 2>&1
