#!/bin/bash
#
# Install system ready for expansion
#
# Usage:
#   install105_forexpansion.sh 10.44.86.105.sh
#
#
...
# DISKS
litp create -t disk -p /infrastructure/systems/sys1/disks/d1 -o name="hdms1" size="12G" bootable="true" uuid="${ms1_disk_uuid}"
...
litp create -t bridge -p /ms/network_interfaces/brnet1vm -o device_name=brnet1vm network_name=net1vm ipaddress="${net1vm_ip_ms}" multicast_snooping=0
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br_vip1       -o device_name=br_vip1 forwarding_delay=0            network_name=net1vm    ipaddress="${VM_net1vm_ip[$i]}" 
...
litp create -t sfs-filesystem -p /infrastructure/storage/storage_providers/sfs_service_sp1/pools/pl1/file_systems/managed2 -o path="/vx/ST105-managed2" size="40M" snap_size=348 cache_name=105cache1
...
litp create -t vm-service -p /software/services/fmmed1 -o service_name=fmmed1 image_name=fmmed cpus=2 ram=2048M internal_status_check=on cleanup_command="/sbin/service fmmed1 force-stop --stop-timeout=30" 
...
# ipv6 only
litp create -t vm-network-interface -p /ms/services/msfmmed1/vm_network_interfaces/net3 -o network_name=834 device_name=eth2 host_device=br1 ipv6addresses="${ms_vm_ipv6_834}"
# vm-alias
...
# vm-disk 
litp create -t vm-disk -p /ms/services/msfmmed1/vm_disks/vm_disk1 -o host_volume_group=vg2 host_file_system=fs1 mount_point=/vm_data_dir
