#!/bin/bash
#
# RHEL7 LITP multi-blade deployment (SAN version)
#
# Usage:
#   RHEL7_deployment_72_multiple_cluster_simplified.sh <CLUSTER_SPEC_FILE>
#
...
litp import /tmp/test_service-1.0-1.noarch.rpm /var/www/html/3pp_rhel7
...
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/jump -t file-system -o type=xfs mount_point=/jump size=1G backup_policy="bu_only"
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/vm_disk_fs -t file-system -o type=xfs size=40M
...
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/root -t file-system -o type=xfs mount_point=/ size=70G snap_size=100
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/var -t file-system -o type=xfs mount_point=/var size=35G snap_size=100 backup_snap_size=50
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/home -t file-system -o type=xfs mount_point=/home size=12G snap_size=100 backup_snap_size=10
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/varlog -t file-system -o type=xfs mount_point=/var/log size=20G
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/varwww -t file-system -o type=xfs mount_point=/var/www size=140G
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/software -t file-system -o type=xfs mount_point=/software size=300G
...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/root -t file-system -o type=xfs mount_point=/ size=8G backup_policy="live"
...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/unmnt_fs -t file-system -o type=xfs size=20M snap_size=0
...
litp create -p /infrastructure/storage/storage_profiles/profile_vm/volume_groups/vg1/file_systems/root -t file-system -o type=xfs mount_point=/ size=8G
...
litp create -p /infrastructure/storage/storage_profiles/profile_vm/volume_groups/vg2/file_systems/vm_images -t file-system -o type=xfs mount_point=/var/lib/libvirt/images size=3500M backup_policy="live"
litp create -p /infrastructure/storage/storage_profiles/profile_vm/volume_groups/vg2/file_systems/unmnt_fs -t file-system -o type=xfs size=20M snap_size=0
...
litp create -p /infrastructure/storage/storage_profiles/profile_vm/volume_groups/vg3/file_systems/vm_instances -t file-system -o type=xfs mount_point=/var/lib/libvirt/instances size=14G
litp create -p /infrastructure/storage/storage_profiles/profile_vm/volume_groups/vg3/file_systems/unmnt_fs -t file-system -o type=xfs size=20M
...
litp create -p /infrastructure/storage/storage_profiles/profile_c3/volume_groups/vg1/file_systems/root -t file-system -o type=xfs mount_point=/ size=8G
...
litp create -p /infrastructure/systems/sys1/disks/disk0 -t disk -o name=hd0 size=1500G bootable=true uuid="${ms_disk_uuid}"
...
litp create -t logrotate-rule -p /ms/configs/logrotate/rules/msg_service -o name="msg_service" path="/var/log/messages" compress=true create=true rotate=5 rotate_every='day' postrotate='/bin/kill -HUP `cat /var/run/syslogd.pid 2> /dev/null` 2> /dev/null || true'

# MS Host Name
litp update -p /ms -o hostname="$ms_host_short"

# VM Service
litp create -t vm-image -p /software/images/image7 -o name="STvm7" source_uri="http://helios/images/enm_base_image.qcow2"
...
# vm-package
litp create -t vm-yum-repo -p /ms/services/not_hyperic/vm_yum_repos/os_repos -o name=os_repo base_url="http://helios/7/os/x86_64"
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
 ...
 if  [ "${cluster[$i]}" !=  "3" ]; then
     ...
     if [ "${cluster[$i]}" =  "1" ]; then
        ...
        litp create -t bond -p /deployments/d1/clusters/c${cluster[$i]}/nodes/n$(($i+1))/network_interfaces/bond07 -o device_name=bond07 mode=1 ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6[$i]}" network_name=mgmt arp_interval=10000 arp_validate=active arp_all_targets=any arp_ip_target="10.44.84.1,10.44.235.1,10.44.86.1,10.44.86.65,10.44.86.129,10.44.86.193,10.44.84.27,10.44.84.28,${sfs_management_ip},10.44.86.31"
     ...
     litp create -t logrotate-rule -p /deployments/d1/clusters/c${cluster[$i]}/nodes/n$(($i+1))/configs/logrotate/rules/msg_service$(($i+1)) -o name="msg_service" path="/var/log/messages" create=true rotate=15 rotate_every='month' postrotate='/bin/kill -HUP `cat /var/run/syslogd.pid 2> /dev/null` 2> /dev/null || true'
...
litp create -t nameserver -p /ms/configs/dns_client/nameservers/dns1 -o ipaddress=10.44.86.212 position=1
...
litp create -t nameserver -p /deployments/d1/clusters/c3/nodes/n5/configs/dns_client/nameservers/dns2 -o ipaddress=10.44.86.212 position=2
...
litp create -t vcs-clustered-service -p /deployments/d1/clusters/c1/services/flying_doves -o active=1 standby=1 name=FO_SG4 online_timeout=100 node_list=n2,n1
...
# PARALLEL SG
litp create -t vcs-clustered-service -p /deployments/d1/clusters/c1/services/cupan_tae -o active=2 standby=0 name=PAR_SG1 online_timeout=70 offline_timeout=140 node_list=n1,n2
litp create -t vcs-clustered-service -p /deployments/d1/clusters/c1/services/anseo -o active=1 standby=0 name=PAR_SG2 online_timeout=50 offline_timeout=110 node_list=n2
...
litp create -t ha-service-config -p /deployments/d1/clusters/c1/services/apache/ha_configs/conf_httpd1 -o status_interval=30 status_timeout=59 restart_limit=2 startup_retry_limit=3 tolerance_limit=2 clean_timeout=58 service_id=httpd_service dependency_list=testservice1
litp create -t ha-service-config -p /deployments/d1/clusters/c1/services/apache/ha_configs/conf_httpd2 -o status_interval=30 status_timeout=70 restart_limit=3 startup_retry_limit=2 tolerance_limit=2 clean_timeout=70 service_id=testservice1
...
litp create -t package -p /software/items/dovecot -o name=dovecot release=8.el7 version=2.2.36 epoch=1
...
litp create -t package -p /software/items/cups-libs -o name=cups-libs version=1.6.3 release=51.el7 epoch=1
for (( i=0; i<2; i++ )); do
...
/usr/bin/md5sum /var/www/html/images/vm_image_rhel6.qcow2 | cut -d ' ' -f 1 > /var/www/html/images/vm_image_rhel6.qcow2.md5
...
# Define 1st 2 VM SGs as FO SGs ............to keep in line ENM Deployment of only 2 FO SGs in the services cluster
  ...
  # vm1 and vm2
      ...
      litp create -t vm-image -p /software/images/image${i} -o name="STvm${i}" source_uri="http://helios/images/vm_image_rhel6.qcow2"
   ...
   litp create -t vm-alias -p /software/services/vmservice${i}/vm_aliases/nfs_storage -o alias_names=nfs address="10.44.86.212"
   ...
   # Add REPOs
   litp create -t vm-yum-repo -p /software/services/vmservice${i}/vm_yum_repos/os -o name=os base_url="http://helios/7/os/x86_64"
   litp create -t vm-yum-repo -p /software/services/vmservice${i}/vm_yum_repos/updates -o name=rhelPatches base_url="http://10.46.83.2/7/updates/x86_64/Packages"
...
litp create -t vm-yum-repo -p /software/services/vmservice1/vm_yum_repos/3pp -o name=3pp base_url="http://helios/3pp_rhel7"
...
litp create -p /deployments/d1/clusters/c3/services/dovecot -t vcs-clustered-service -o active=1 standby=0 name=PL_AMOS online_timeout=59 node_list=n5
litp inherit -p /deployments/d1/clusters/c3/services/dovecot/applications/dovecot -s /software/services/dovecot
litp create -t ha-service-config -p /deployments/d1/clusters/c3/services/dovecot/ha_configs/conf_dovecot1 -o status_interval=30 status_timeout=79 restart_limit=2 startup_retry_limit=3 tolerance_limit=2 clean_timeout=58 service_id=dovecot
