...
# NOT TESTED - updated for new network config but not tested yet
#
# CMW - 2 nics for tipc
# 4 nics on MN
# 2 nics on MS
...
#
# Usage:
#   ST_Deployment_6.sh <CLUSTER_SPEC_FILE>
#
...
litp create -p /software/items/ntp1/servers/server0 -t ntp-server -o server="${ntp_ip[0]}"
...
litp create -p /infrastructure/networking/network_profiles/np2/networks/data -t network -o network_name=data interface=nic1
litp create -p /infrastructure/networking/network_profiles/np2/interfaces/nic1 -t interface -o interface_basename=eth1
