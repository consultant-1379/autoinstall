...
litp create -p /infrastructure/networking/ip_routes/r1 -t network-route -o name="default" subnet="0.0.0.0/0" gateway="${nodes_gateway}"
...
litp link -p /ms/ip_routes/r1 -t network-route -o name="default"
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp link -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/ip_routes/r1 -t network-route -o name="default"
done
