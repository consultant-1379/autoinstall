...
litp create -p /ms/network_interfaces/b0_vlan -t vlan -o device_name=bondmgmt.${ms_vlan_id} ipaddress="${ms_ip}" ipv6address="${ms_ipv6_00}" network_name=mgmt
...
##NB: RICCI IS HARDCODED, IF CHANGING PARALLEL SG UPDATE THIS.
litp create -p /deployments/d1/clusters/c1/services/"${FO_SG_pkg[$x]}" -t vcs-clustered-service -o active=1 standby=1 name=FO_vcs$(($x+1)) online_timeout=45 node_list='n1,n2' dependency_list=ricci
