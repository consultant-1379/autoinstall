...
litp create -p /deployments/d1/clusters/AMOS -t vcs-cluster -o cluster_type=sfha low_prio_net=mgmt llt_nets='hb1,hb2' cluster_id=6052

# CLUSTER CONFIGURATION FOR FILEWALLS
litp create -p /deployments/d1/clusters/AMOS/configs/fw_config_init -t firewall-cluster-config
litp create -p /deployments/d1/clusters/AMOS/configs/fw_config_init/rules/fw_icmp -t firewall-rule -o name="100 icmp" proto="icmp"
...
litp create -p /deployments/d1/clusters/AMOS/services/httpd -t vcs-clustered-service -o active=1 standby=1 name=FO_vcs1 online_timeout=45 node_list=n3,n4 dependency_list=cups
litp create -p /deployments/d1/clusters/AMOS/services/httpd/runtimes/httpd -t lsb-runtime -o name=httpd service_name=httpd status_interval=100
litp inherit -p /deployments/d1/clusters/AMOS/services/httpd/runtimes/httpd/packages/pkg1 -s /software/items/httpd
litp create -p /deployments/d1/clusters/AMOS/services/cups -t vcs-clustered-service -o active=2 standby=0 name=PL_vcs1 node_list=n4,n3
litp create -p /deployments/d1/clusters/AMOS/services/cups/ha_configs/conf1 -t ha-service-config -o status_interval=10 status_timeout=10 restart_limit=5 startup_retry_limit=2
...
litp inherit -p /deployments/d1/clusters/AMOS/services/cups/applications/cups -s /software/services/cups

litp create -t blade -p /infrastructure/systems/sys5 -o system_name="sys5"

litp create -t node -p /deployments/d1/clusters/AMOS/nodes/n4 -o hostname=node4dot61 node_id=4

litp create -t eth -p /deployments/d1/clusters/AMOS/nodes/n4/network_interfaces/nic_0 -o device_name=eth0 macaddress=2c:59:e5:3d:32:58 network_name=mgmt ipaddress=10.44.235.48
...
litp inherit -p /deployments/d1/clusters/AMOS/nodes/n4/system -s /infrastructure/systems/sys5
litp inherit -p /deployments/d1/clusters/AMOS/nodes/n4/os -s /software/profiles/os_prof1
litp inherit -p /deployments/d1/clusters/AMOS/nodes/n4/storage_profile -s /infrastructure/storage/storage_profiles/profile_1

litp inherit -p /deployments/d1/clusters/AMOS/nodes/n4/routes/r1 -s /infrastructure/networking/routes/r1
litp inherit -p /deployments/d1/clusters/AMOS/nodes/n4/routes/r5 -s /infrastructure/networking/routes/r1 -o subnet=10.44.84.0/24 gateway=10.44.235.1
