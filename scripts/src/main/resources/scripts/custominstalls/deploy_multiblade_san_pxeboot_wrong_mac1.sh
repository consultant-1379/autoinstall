...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    # SETUP PXE BOOT INTERFACE AND BOND/BRIDGE as example
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_eth0_mac[$i]}" pxe_boot_only=true
    ...
    litp create -t bond -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/bond0 -o device_name="bond0" mode=1 miimon=100 network_name="mgmt" ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}"
    ...
    # TRAFFIC NETWORKS
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if4 -t eth -o device_name=eth4 macaddress="00:00:11:22:33:0$i" network_name='traffic1' ipaddress="${node_ip_2[$i]}"
