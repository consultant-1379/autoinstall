...
#litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/mount1 -o export_path="${sfs_prefix}-managedfs1" provider="virtserv1" mount_point="/system61_mg" mount_options="soft,intr" network_name="mgmt"
#litp create -t nfs-mount -p /infrastructure/storage/nfs_mounts/mount2 -o export_path="${sfs_prefix}-managedfs2" provider="virtserv1" mount_point="/cluster61_mg" mount_options="soft,intr" network_name="mgmt"
...
x=11
x=$[$x+1] VM_cpu[x]=3; VM_ram[x]=2000M; VM_active[x]=1; VM_standby[x]=1 VM_node_list[x]="n1,n2"  
...
x=$[$x+1] VM_cpu[x]=7; VM_ram[x]=4000M;  VM_active[x]=1; VM_standby[x]=1 VM_node_list[x]="n2,n1" 
...
# increase VM13's online_timeout so that only VM has the short default timeout
# so this plan will always fail as vm12 will fail to come online in time
