...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}" network_name='mgmt' multicast_snooping=0 multicast_querier=0 multicast_router=0 forwarding_delay=7     # HEARTBEAT NETWORK SETUP
