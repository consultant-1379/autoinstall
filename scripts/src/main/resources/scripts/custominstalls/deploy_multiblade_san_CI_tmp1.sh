...
##NB: HTTPd IS HARDCODED, IF CHANGING PARALLEL SG UPDATE THIS.
litp create -p /deployments/d1/clusters/c1/services/"${FO_SG_pkg[$x]}" -t vcs-clustered-service -o active=1 standby=1 name=FO_vcs$(($x+1)) online_timeout=45 node_list='n1,n2' dependency_list=httpd
...
litp inherit -p /deployments/d1/clusters/c1/services/"${FO_SG_pkg[$x]}"/runtimes/"${FO_SG_pkg[$x]}"/filesystems/fs1 -s /deployments/d1/clusters/c1/storage_profile/vxvm_profile/volume_groups/vg1_vxvm/file_systems/data1_vxvm
done
...
litp create  -p /deployments/d1/clusters/c1/services/"${PL_SG_pkg[$x]}"/ipaddresses/ip1 -t vip -o ipaddress="${nodes_sg_fo1_vip1}" network_name=traffic3
litp create  -p /deployments/d1/clusters/c1/services/"${PL_SG_pkg[$x]}"/ipaddresses/ip2 -t vip -o ipaddress="${nodes_sg_fo1_vip2}" network_name=traffic3
done

# Add one VM as Parrallel SG
litp create -p /software/images/image1 -t vm-image -o name="fmmed" source_uri=http://"${ms_host}"/images/image.qcow2
litp create -p /software/services/fmmed1 -t vm-service -o service_name=fmmed1 image_name=fmmed cpus=2 ram=256M internal_status_check=off
litp create -p /deployments/d1/clusters/c1/services/fmmed1 -t vcs-clustered-service -o name=fmmed1 active=1 standby=0 node_list='n1' online_timeout=3000
litp inherit -p /deployments/d1/clusters/c1/services/fmmed1/applications/fmmed -s /software/services/fmmed1
litp create -p /software/services/fmmed1/vm_network_interfaces/net1 -t vm-network-interface -o network_name=mgmt device_name=eth0 host_device=br0 ipaddresses="${vm_ip_835}"
