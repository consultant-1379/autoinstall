#!/bin/bash
#
# RHEL7 based on ST_Deployment_4_inherit
#
# Deployment used for 10.44.86.86
#
# Usage:
#   RHEL7_deployment_86_simplified.sh <CLUSTER_SPEC_FILE>
#
...
litp import /tmp/lsb_pkg 3pp_rhel7
...
litp create -t ntp-server -p /software/items/ntp2/servers/server1 -o server=10.44.86.212
...
litp create  -t disk                -p /infrastructure/systems/sys1/disks/d1 -o name="hd1" size=1200G bootable="true" uuid=$ms_disk_uuid
...
litp create  -t file-system         -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/fs1 -o type="xfs" mount_point="/mount_ms_fs1" size="100M" snap_size="5" snap_external="false" backup_snap_size=5
...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/root                         -t file-system     -o type=xfs mount_point=/                size=8G   snap_size=50
...
# KS FS modelling
litp create -t file-system -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/root -o type=xfs mount_point=/ size=70G snap_size=100 backup_snap_size=100
litp create -t file-system -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/home -o type=xfs mount_point=/home size=12G snap_size=100 backup_snap_size=100
litp create -t file-system -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/var_log -o type=xfs mount_point=/var/log size=20G snap_size=0 backup_snap_size=100
litp create -t file-system -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/var_www -o type=xfs mount_point=/var/www size=140G snap_size=100 backup_snap_size=100
litp create -t file-system -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/var -o type=xfs mount_point=/var size=35G snap_size=100 backup_snap_size=100
litp create -t file-system -p /infrastructure/storage/storage_profiles/sp1/volume_groups/vg1/file_systems/software -o type=xfs mount_point=/software size=300G snap_size=0 backup_snap_size=0
for (( i=0; i<6; i++ )); do
    litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/LVM_VG1_FS$i -t file-system -o type=xfs mount_point=/LVM_mp_VG1_FS$i size=200M snap_size=$((100-($i * 10))) backup_snap_size=$((100-($i * 10)))
done


litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/LVM_no_snap -t file-system -o type=xfs mount_point=/LVM_no_snap size=100M snap_size=5 snap_external=true


# Unmounted FS1 for LVM volume group vg1
litp create -t file-system -p  /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/unmounted_fs1 -o type=xfs size=52M snap_size=30 backup_snap_size=30


# Unmounted FS2 for LVM volume group vg1
litp create -t file-system -p  /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/unmounted_fs2 -o type=xfs size=52M snap_size=30 backup_snap_size=30
...
litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/nm1        -o export_path="${nfs_prefix}/dir_share_86_A" provider="nfs1"      mount_point="/nfs_dirA"      mount_options="soft,intr" network_name="mgmt"
litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/nm2        -o export_path="${nfs_prefix}/dir_share_86_B" provider="nfs1"      mount_point="/nfs_dirB"      mount_options="soft,intr" network_name="mgmt"
litp create -t nfs-mount            -p /infrastructure/storage/nfs_mounts/nm3        -o export_path="${nfs_prefix}/dir_share_86_C" provider="nfs1"      mount_point="/nfs_dirC"      mount_options="soft,intr" network_name="mgmt"
...
litp create -t nameserver -p /ms/configs/dns_client/nameservers/my_name_server_B -o ipaddress=10.44.86.212 position=2
...
litp create -t nameserver -p /deployments/d1/clusters/c1/nodes/n1/configs/dns_client/nameservers/my_name_server_A -o ipaddress=10.44.86.212 position=1
litp create -t nameserver -p /deployments/d1/clusters/c1/nodes/n1/configs/dns_client/nameservers/my_name_server_B -o ipaddress=2001:4860:0:1001::68 position=2
...
litp create -t nameserver -p /deployments/d1/clusters/c1/nodes/n2/configs/dns_client/nameservers/my_name_server_A -o ipaddress=10.44.86.212 position=1
litp create -t nameserver -p /deployments/d1/clusters/c1/nodes/n2/configs/dns_client/nameservers/my_name_server_B -o ipaddress=2001:4860:0:1001::68 position=2
...
# Fencing disks
litp create -t disk -p /deployments/d1/clusters/c1/fencing_disks/fd1 -o uuid="${fd1_uuid}" size=90M name=fencing_disk_1
litp create -t disk -p /deployments/d1/clusters/c1/fencing_disks/fd2 -o uuid="${fd2_uuid}" size=90M name=fencing_disk_2
litp create -t disk -p /deployments/d1/clusters/c1/fencing_disks/fd3 -o uuid="${fd3_uuid}" size=90M name=fencing_disk_3
...
# lsb service
x=0
pkg[x]="httpd";  VIP_count[x]=$[5*2]; active[x]=2; standby[x]=0 status_interval[x]=20   status_timeout[x]=30    restart_limit[x]=30     startup_retry_limit[x]=40    node_list[x]="n2,n1" initial_online_dependency_list[x]="cups" x=$[$x+1]
pkg[x]="cups";  VIP_count[x]=3;      active[x]=1; standby[x]=1 status_interval[x]=40   status_timeout[x]=30    restart_limit[x]=3      startup_retry_limit[x]=2     node_list[x]="n2,n1" initial_online_dependency_list[x]="" x=$[$x+1]

vip_count=1
for (( x=0; x<${#pkg[@]}; x++ )); do
litp create -t package               -p /software/items/"${pkg[$x]}" -o name="${pkg[$x]}"
litp create -t vcs-clustered-service -p /deployments/d1/clusters/c1/services/"${pkg[$x]}" -o active="${active[$x]}" standby="${standby[$x]}" name=vcs_"${pkg[$x]}" online_timeout=45 offline_timeout=45 node_list="${node_list[$x]}" initial_online_dependency_list="${initial_online_dependency_list[$x]}"
...
litp inherit -p /deployments/d1/clusters/c1/services/cups/filesystems/fs3     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_3/file_systems/VxVM_VG3_3
litp inherit -p /deployments/d1/clusters/c1/services/cups/filesystems/fs4     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_4/file_systems/VxVM_VG3_4
litp inherit -p /deployments/d1/clusters/c1/services/cups/filesystems/fs5     -s /deployments/d1/clusters/c1/storage_profile/sp3/volume_groups/vg_vxvm_5/file_systems/VxVM_VG3_5
litp inherit -p /deployments/d1/clusters/c1/services/cups/filesystems/fs7     -s /deployments/d1/clusters/c1/storage_profile/sp5/volume_groups/vg_vxvm_0_snap/file_systems/No_snap
...
litp create -t package -p /software/items/cups-libs        -o name=cups-libs version=1.6.3  release=51.el7
...
for (( i=1; i<=${#VM_cpu[@]}; i++ )); do
    litp create -t vm-image    -p /software/images/id_image$i -o name="image_vm$i" source_uri=http://"${ms_host}"/images/vm_image_rhel7.qcow2
    ...
    litp create -t vm-yum-repo -p /software/services/se_vm$i/vm_yum_repos/updates -o name=vm_UPDATES base_url="http://${ms_host}/7/updates/x86_64/Packages"
    litp create -t vm-yum-repo -p /software/services/se_vm$i/vm_yum_repos/os -o name=vm_os base_url=http://${ms_ip}/7/os/x86_64
...
litp update -p /software/images/id_image2 -o name="image_vm2" source_uri=http://"${ms_host}"/images/vm_image_rhel6.qcow2
litp update -p /software/images/id_image4 -o name="image_vm4" source_uri=http://"${ms_host}"/images/vm_image_rhel6.qcow2
...
litp create -p /infrastructure/storage/storage_profiles/profile_21/volume_groups/vg1/file_systems/root -t file-system -o type=xfs mount_point=/ size=10G snap_size=50
