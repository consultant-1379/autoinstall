...
#on nodes instead of cluster
#litp create -p /deployments/d1/clusters/c1/configs/fw_config_init/rules/fw_icmpv6 -t firewall-rule -o name="099 icmpv6" proto="ipv6-icmp" provider=ip6tables
...
#litp create -p /ms/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${ms_ip}" ipv6address="${ms_ipv6_00}" network_name=mgmt stp=true forwarding_delay=30
...
#litp create -t nameserver -p /ms/configs/dns_client/nameservers/init_name_server -o ipaddress="${nameserver_ip}" position=1
...
#litp create -p /infrastructure/networking/routes/traffic3_gw_n$(($i+1)) -t route -o subnet=${traffic_network3_gw_subnet} gateway=${traffic_network3_gw$(($i+1))}
...
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh1 -o network_name="mgmt"     ip="${vcs_network_host1}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh2 -o network_name="mgmt"     ip="${ms_ip}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh3 -o network_name="mgmt"     ip="${vcs_network_host3}" 
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh4 -o network_name="traffic1" ip="${ms_ip}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh5 -o network_name="traffic1" ip="${vcs_network_host5}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh6 -o network_name="traffic1" ip="${vcs_network_host6}" 
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh7 -o network_name="traffic1" ip="${vcs_network_host7}" 
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh8 -o network_name="traffic2" ip="${vcs_network_host8}" 
...
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh10 -o network_name="traffic2" ip="${vcs_network_host10}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh11 -o network_name="traffic1" ip="${vcs_network_host11}"
...
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh16 -o network_name=traffic3 ip=${traffic_network3_gw}
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh17 -o network_name=traffic3 ip="${node_ip_4[1]}"

litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh18 -o network_name=traffic1 ip="${node_ip_2[2]}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh19 -o network_name=traffic2 ip="${node_ip_3[2]}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh20 -o network_name=traffic3 ip="${node_ip_4[2]}"

litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh21 -o network_name=traffic1 ip="${node_ip_2[3]}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh22 -o network_name=traffic2 ip="${node_ip_3[3]}"
litp create -t vcs-network-host -p /deployments/d1/clusters/c1/network_hosts/nh23 -o network_name=traffic3 ip="${node_ip_4[3]}"
...
####
    #litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_eth0_mac[$i]}" ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}" network_name=mgmt
...
litp create -p /deployments/d1/clusters/c1/services/httpd -t vcs-clustered-service -o active=1 standby=0 name=PL_vcs1 node_list='n1'
...
litp create  -p /deployments/d1/clusters/c1/services/httpd/ipaddresses/ip1 -t vip -o ipaddress="${nodes_sg_fo1_vip1}" network_name=traffic3
litp create  -p /deployments/d1/clusters/c1/services/httpd/ipaddresses/ip2 -t vip -o ipaddress="${nodes_sg_fo1_vip2}" network_name=traffic3
done
