...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/fs1 -t file-system -o type="ext4" mount_point="/data_dir1" size="200M" snap_size="5" backup_snap_size="10" snap_external="false"
...
# Add extra LVM
litp create -t file-system -p /infrastructure/storage/storage_profiles/spms/volume_groups/vg1/file_systems/fs1 -o type="ext4" mount_point="/data_dir1" size="200M" snap_size="5" backup_snap_size="10" snap_external="false"
litp create -t file-system -p /infrastructure/storage/storage_profiles/spms/volume_groups/vg1/file_systems/fs2 -o type="ext4" mount_point="/data_dir2" size="200M" snap_size="5" backup_snap_size="10" snap_external="false"
...
litp create -t bridge -p /ms/network_interfaces/br0 -o device_name=br0 ipaddress="${ms_ip}" ipv6address="${ms_ipv6_835}" network_name=mgmt multicast_snooping=1 multicast_querier=1 multicast_router=2 hash_max=2048
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -t bridge -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br_834  -o device_name=br_834 forwarding_delay=0 network_name=ipv61 ipv6address="${node_ipv6_834[$i]}" multicast_snooping=1 multicast_querier=1 multicast_router=2 hash_max=2048
