...
then
    ...
    litp create -p /ms/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${ms_eth0_mac}" master=bondmgmt rx_ring_buffer=512 tx_ring_buffer=1024
    litp create -p /ms/network_interfaces/if1 -t eth -o device_name=eth1 macaddress="${ms_eth1_mac}" master=bondmgmt rx_ring_buffer=1024 tx_ring_buffer=512
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_eth0_mac[$i]}" ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}" rx_ring_buffer=1024 tx_ring_buffer=512 network_name='mgmt'
    # BRIDGE ETH0
