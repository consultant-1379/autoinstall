...
# RHEL7 version of deploy_multiblade_san_mcast_va.sh
#
# Usage:
#   RHEL7_deploy_multiblade_san_mcast_va.sh <CLUSTER_SPEC_FILE>
#
...
litp create -t package -p /software/items/dovecot -o name=dovecot
...
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/dataA -t file-system -o type=xfs mount_point=/dataA size=400M
litp create -p /infrastructure/storage/storage_profiles/profile_ms/volume_groups/vg1/file_systems/data_umount -t file-system -o type=xfs size=48M
...
litp create -p /deployments/d1/clusters/c1/services/"$SL_SG_pkg"/ha_configs/conf1 -t ha-service-config -o service_id="dovecot" status_interval=120 status_timeout=50 restart_limit=10 startup_retry_limit=3 fault_on_monitor_timeouts=5 tolerance_limit=15 clean_timeout=60
litp inherit -p /deployments/d1/clusters/c1/services/"$SL_SG_pkg"/applications/dovecot -s /software/services/dovecot
