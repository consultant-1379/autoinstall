...
for (( x=0; x<${#FO_SG_pkg[@]}; x++ )); do 
    litp create -p /deployments/d1/clusters/c1/services/"${FO_SG_pkg[$x]}" -t vcs-clustered-service -o active=1 standby=1 name=FO_vcs$(($x+1)) online_timeout=45 node_list='n2,n1' dependency_list=httpd
