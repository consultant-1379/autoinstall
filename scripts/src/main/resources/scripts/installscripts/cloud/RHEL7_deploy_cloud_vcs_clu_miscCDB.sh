#!/bin/bash
#
# LITP cloud deployment script for RHEL7
#   cluster_type=vcs
#
# Usage:
#   RHEL7_deploy_cloud.sh <CLUSTER_SPEC_FILE>
#
# This script is for RHEL7 miscCDB.
# There is a dns setting that leads to errors with
# the vmmonitord test case.
...
litp create -p /infrastructure/storage/storage_profiles/profile_1/volume_groups/vg1/file_systems/root -t file-system -o type=xfs mount_point=/ size=16G
...
litp create -p /ms/network_interfaces/if3 -t eth -o device_name=eth1 macaddress="${ms_eth3_mac}" master=bondmgmt
...
# DNS FOR MS
litp create -p /ms/configs/dns_client -t dns-client -o search=ammeonvpn.com,exampleonejunk.com,exampletwojunk.com,examplethreejunk.com,examplefourjunk.com,examplefivejunk.com
...
litp create -p /deployments/d1/clusters/c1 -t vcs-cluster -o cluster_type=vcs low_prio_net=mgmt llt_nets='hb1,hb2' cluster_id="${cluster_id}"
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${node_ip[$i]}" ipv6address="${node_ipv6_00[$i]}" network_name='mgmt'

    # HEARTBEAT NETWORK SETUP
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if7 -t eth -o device_name=eth7 macaddress="${node_eth7_mac[$i]}" network_name=hb1
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/if8 -t eth -o device_name=eth8 macaddress="${node_eth8_mac[$i]}" network_name=hb2
    ...
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/network_interfaces/br6 -t bridge -o device_name='br6' network_name=dhcp_network ipaddress="${dhcp_ip_1[$i]}"
    ...
    # DNS SETUP FOR NODES
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/configs/dns_client -t dns-client -o search=ammeonvpn.com,exampleonejunk.com,exampletwojunk.com,examplethreejunk.com,examplefourjunk.com,examplefivejunk.com
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/configs/dns_client/nameservers/gw_name_server -t nameserver -o ipaddress=${ms_gateway} position=1
    litp create -p /deployments/d1/clusters/c1/nodes/n$(($i+1))/configs/dns_client/nameservers/init_name_server -t nameserver -o ipaddress=10.44.86.212 position=2
...
for (( x=0; x<${#FO_SG_pkg[@]}; x++ )); do 
    litp create -p /deployments/d1/clusters/c1/services/"${FO_SG_pkg[$x]}" -t vcs-clustered-service -o active=1 standby=1 name=FO_vcs$(($x+1)) offline_timeout=10 online_timeout=30 node_list='n2,n1' dependency_list=httpd
...
for (( x=0; x<${#PL_SG_pkg[@]}; x++ )); do
    litp create -p /deployments/d1/clusters/c1/services/"${PL_SG_pkg[$x]}" -t vcs-clustered-service -o active=2 standby=0 name=PL_vcs$(($x+1)) node_list='n1,n2' offline_timeout=10 online_timeout=30
