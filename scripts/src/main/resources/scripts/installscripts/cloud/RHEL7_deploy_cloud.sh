...
# MANAGED SFS MOUNTS
litp create -p /infrastructure/storage/nfs_mounts/mount2 -t nfs-mount -o export_path="${managedfs1}" provider="virtserv1" mount_point="/mfs_cluster1" mount_options="soft" network_name="mgmt"
litp create -p /infrastructure/storage/nfs_mounts/mount3 -t nfs-mount -o export_path="${managedfs2}" provider="virtserv1" mount_point="/mfs_cluster2" mount_options="soft" network_name="mgmt"
...
litp create -p /ms/network_interfaces/if3 -t eth -o device_name=eth3 macaddress="${ms_eth3_mac}" master=bondmgmt
...
for (( i=0; i<${#node_sysname[@]}; i++ )); do
    ...
    # DNS SETUP FOR NODES
    # (RHEL7 DNS issue TORF-463684 TORF-474639 )
