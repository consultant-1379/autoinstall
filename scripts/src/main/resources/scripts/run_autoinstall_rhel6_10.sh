...
#python autoinstall.py -litpms http://10.44.84.100/iso/RHEL//rhel-server-6.4-x86_64-dvd.iso /home/admin/iso_files/2.x/ERIClitp_CXP9024296-1.0.232.iso /home/admin/iso_files/3pp//core-3pps-1.0.7.gz singleblade /opt/ericsson/SIT/autoinstall/autoinstall_2.1/install_scripts/singleblade/deploy_singleblade.sh /opt/ericsson/SIT/autoinstall/autoinstall_2.1/clusterfiles/singleblade/10.44.86.46.sh http://10.^C.86.30/iso/autoinstall_jobs/ /home/admin/iso_files/autoinstall_jobs/
...
TARBALL_DIR="/home/admin/iso_files/3pp/"
if [ $TARBALL_FILE == "LATEST" ]
then
    tarball_name=( $(ls -lrt $TARBALL_DIR | grep 'core-3pps' | awk '{print $NF}' | tail -1) )
...
if [ $UPGRADE_3PP ] && $UPGRADE_3PP
then
    upgrade_3pp="--upgrade_3pp"
else
    upgrade_3pp=""
fi
...
then
    if $OS_PATCHES
    then
        python autoinstall_rhel6_10.py -ci --rheliso=$RHEL_ISO --litpiso=$LITP_ISO_PATH --thirdpp=$TARBALL_PATH --install_type=$ENVIRONMENT --install_script=$LITP_INSTALL_SCRIPT --cluster_file=$LITP_DATAREG --server_http_location=$AI_HTTP_LOCATION --server_http_full_path_location=$AI_CLI_LOCATION --install_option=$DEPLOYMENT_SCRIPT_TYPE --with_os_patches=$OS_PATCHES_PATH $retry_plan $upgrade_3pp $RETRY_SCRIPT_PLAN $reboot_peer_nodes $vcs_debug_logging $skip_peer_node_passwd $skip_install_snapshot $expect_large_plan $pre_xml_load_script $local_trust_auth $skip_post_deploy_snapshot
    else
        python autoinstall_rhel6_10.py -ci --rheliso=$RHEL_ISO --litpiso=$LITP_ISO_PATH --thirdpp=$TARBALL_PATH --install_type=$ENVIRONMENT --install_script=$LITP_INSTALL_SCRIPT --cluster_file=$LITP_DATAREG --server_http_location=$AI_HTTP_LOCATION --server_http_full_path_location=$AI_CLI_LOCATION --install_option=$DEPLOYMENT_SCRIPT_TYPE $retry_plan $upgrade_3pp $RETRY_SCRIPT_PLAN $reboot_peer_nodes $vcs_debug_logging $skip_peer_node_passwd $skip_install_snapshot $expect_large_plan $pre_xml_load_script $local_trust_auth $skip_post_deploy_snapshot
    fi
    retval=( $(echo "$?") )
    if [ $retval -ne 0 ]
    then
...
then
    if $OS_PATCHES
    then
        python autoinstall_rhel6_10.py -litpms --rheliso=$RHEL_ISO --litpiso=$LITP_ISO_PATH --thirdpp=$TARBALL_PATH --install_type=$ENVIRONMENT --install_script=$LITP_INSTALL_SCRIPT --cluster_file=$LITP_DATAREG --server_http_location=$AI_HTTP_LOCATION --server_http_full_path_location=$AI_CLI_LOCATION --install_option=$DEPLOYMENT_SCRIPT_TYPE --with_os_patches=$OS_PATCHES_PATH $retry_plan $RETRY_SCRIPT_PLAN $local_trust_auth
    else
...
then
    if $OS_PATCHES
    then
        python autoinstall_rhel6_10.py -litpc --litpiso=$LITP_ISO_PATH --thirdpp=$TARBALL_PATH --install_type=$ENVIRONMENT --install_script=$LITP_INSTALL_SCRIPT --cluster_file=$LITP_DATAREG --server_http_full_path_location=$AI_CLI_LOCATION --install_option=$DEPLOYMENT_SCRIPT_TYPE --with_os_patches=$OS_PATCHES_PATH $retry_plan $RETRY_SCRIPT_PLAN $reboot_peer_nodes $vcs_debug_logging $skip_peer_node_passwd $skip_install_snapshot $expect_large_plan $pre_xml_load_script $local_trust_auth $skip_post_deploy_snapshot
    else
...
then
    python autoinstall_rhel6_10.py -rhel --rheliso=$RHEL_ISO --litpiso=$LITP_ISO_PATH --thirdpp=$TARBALL_PATH --install_type=$ENVIRONMENT --install_script=$LITP_INSTALL_SCRIPT --cluster_file=$LITP_DATAREG --server_http_location=$AI_HTTP_LOCATION --server_http_full_path_location=$AI_CLI_LOCATION --install_option=$DEPLOYMENT_SCRIPT_TYPE $retry_plan $RETRY_SCRIPT_PLAN
    retval=( $(echo "$?") )
    if [ $retval -ne 0 ]
    then
        echo ""
...
then
    if $OS_PATCHES
    then
        python autoinstall_rhel6_10.py -rhellitp --litpiso=$LITP_ISO_PATH --thirdpp=$TARBALL_PATH --install_type=$ENVIRONMENT --install_script=$LITP_INSTALL_SCRIPT --cluster_file=$LITP_DATAREG --server_http_full_path_location=$AI_CLI_LOCATION --install_option=$DEPLOYMENT_SCRIPT_TYPE --with_os_patches=$OS_PATCHES_PATH $retry_plan $RETRY_SCRIPT_PLAN $skip_install_snapshot $expect_large_plan $pre_xml_load_script $local_trust_auth $skip_post_deploy_snapshot
    else
