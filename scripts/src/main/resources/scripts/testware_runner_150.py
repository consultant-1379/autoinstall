...
# GLOBAL VARIABLES
RHEL_ISO = "rhel-server-6.10-x86_64-dvd.iso"
OS_PATCHES_INSTALL = "http://10.44.86.30/iso/RHEL_Patches/6.6_RevAC/RHEL_OS_Patch_Set_CXP9026826-1.22.3.tar.gz"
OS_PATCHES_UPGRADE = "http://10.44.86.30/iso/RHEL_Patches/6.6_RevAD/RHEL_OS_Patch_Set_CXP9026826-1.23.4.tar.gz"
#OS_PATCHES_UPGRADE = "https://arm1s11-eiffel004.eiffel.gic.ericsson.se:8443/nexus/content/repositories/iso/com/ericsson/nms/litp/RHEL_OS_Patch_Set_CXP9034997/2.2.1/RHEL_OS_Patch_Set_CXP9034997-2.2.1.iso"
...
class ToolsDownload:
    ...
    def download_given_tools(self, rhel_only=False):
        ...
        if rhel_only:
            ...
            if self.job_details["litp_iso_version"]:
                ...
                self.job_details["core_3pp_nexus_link"] = self.wget_tool(self.job_details["core_3pp_nexus_link"], litp_3pp_download)
            ...
            if self.job_details["litp_iso_version"] or self.job_details["litp_upgrade_iso_version"] or self.job_details["expansionScript"] or self.job_details["litp_backup_restore"] or restore_snapshot:
                if self.job_details["local_autoinstall"]:
                    self.job_details["autoinstall_tarball_nexus_link"] = self.job_details["local_autoinstall"]
                else:
                    self.job_details["autoinstall_tarball_nexus_link"] = self.wget_tool(self.job_details["autoinstall_tarball_nexus_link"], autoinstall_code)
    ...
    def run_autoinstall(self, only_test=False):
        ...
        autoinstall_command += " --thirdpp={0}".format(self.job_details["core_3pp_nexus_link"])
    ...
    def run_autoupgrade(self):
        ...
        if get_env("SKIP_UPGRADE_SNAPSHOTS") == "true":
            autoinstall_command += " --skip_upgrade_snapshot"
    ...
    def run_restore_snapshot(self):
        """
        Generate the command arguments to restore an unnamed snapshot
        """
        autoinstall_code = self.job_details["autoinstall_tarball_nexus_link"] + "autoinstall.py"
        autoinstall_command = "{0} {1}".format(self.python_command, autoinstall_code)
        autoinstall_command += " {0}".format("--restore_snapshot")
...
job_details["core_3pp_nexus_link"] = get_env("core_3pp_nexus_link")
