...
litp create -p /deployments/d1/clusters/c1/nodes/n3/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${node_expansion_ip[0]}" ipv6address="${node_expansion_ipv6_00[0]}" forwarding_delay=0 network_name='mgmt' stp=true forwarding_delay=30
...
litp create -p /deployments/d1/clusters/c1/nodes/n3/configs/fw_config_init/rules/fw_icmp_ip6 -t firewall-rule -o 'name=099 icmpipv6' proto=ipv6-icmp
...
#litp inherit -p /deployments/d1/clusters/c1/nodes/n3/routes/traffic1_gw -s /infrastructure/networking/routes/traffic1_gw
...
##ADD SFS
litp inherit -p /deployments/d1/clusters/c1/nodes/n3/file_systems/fs1 -s /infrastructure/storage/nfs_mounts/mount1
...
litp create -p /deployments/d1/clusters/c1/nodes/n4 -t node -o hostname="${node_expansion_hostname[1]}"

litp inherit -p /deployments/d1/clusters/c1/nodes/n4/system -s  /infrastructure/systems/sys5
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/os -s /software/profiles/os_prof1  
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/storage_profile -s /infrastructure/storage/storage_profiles/profile_1 
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/items/ntp1 -s /software/items/ntp1
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/items/java -s /software/items/openjdk

litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/if0 -t eth -o device_name=eth0 macaddress="${node_expansion_eth0_mac[1]}" bridge='br0'
litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/br0 -t bridge -o device_name=br0 ipaddress="${node_expansion_ip[1]}" ipv6address="${node_expansion_ipv6_00[1]}" forwarding_delay=0 network_name='mgmt' stp=true forwarding_delay=30
litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/if2 -t eth -o device_name=eth2 macaddress="${node_expansion_eth2_mac[1]}" network_name=hb1
litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/if3 -t eth -o device_name=eth3 macaddress="${node_expansion_eth3_mac[1]}" network_name=hb2
litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/if4 -t eth -o device_name=eth4 macaddress="${node_expansion_eth4_mac[1]}" network_name='traffic1' ipaddress="${node_expansion_ip_2[1]}"
litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/if5 -t eth -o device_name=eth5 macaddress="${node_expansion_eth5_mac[1]}" network_name='traffic2' ipaddress="${node_expansion_ip_3[1]}" 
litp create -p /deployments/d1/clusters/c1/nodes/n4/network_interfaces/if6 -t eth -o device_name=eth6 macaddress="${node_expansion_eth6_mac[1]}" network_name='traffic3' ipaddress="${node_expansion_ip_4[1]}"

#Add firewalls
litp create -p /deployments/d1/clusters/c1/nodes/n4/configs/fw_config_init -t firewall-node-config 
litp create -p /deployments/d1/clusters/c1/nodes/n4/configs/fw_config_init/rules/fw_nfsudp -t firewall-rule -o 'name=011 nfsudp' dport=111,2049,4001 proto=udp
litp create -p /deployments/d1/clusters/c1/nodes/n4/configs/fw_config_init/rules/fw_nfstcp -t firewall-rule -o 'name=001 nfstcp' dport=111,2049,4001 proto=tcp
litp create -p /deployments/d1/clusters/c1/nodes/n4/configs/fw_config_init/rules/fw_icmp_ip6 -t firewall-rule -o 'name=099 icmpipv6' proto=ipv6-icmp

litp inherit -p /deployments/d1/clusters/c1/nodes/n4/routes/r1 -s /infrastructure/networking/routes/r1

##IPV6 route
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/routes/r2_ipv6 -s /infrastructure/networking/routes/default_ipv6

#litp inherit -p /deployments/d1/clusters/c1/nodes/n4/routes/traffic1_gw -s /infrastructure/networking/routes/traffic1_gw
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/routes/traffic2_gw -s /infrastructure/networking/routes/traffic2_gw
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/routes/traffic3_gw -s /infrastructure/networking/routes/traffic3_gw_n4

##ADD SFS
litp inherit -p /deployments/d1/clusters/c1/nodes/n4/file_systems/fs1 -s /infrastructure/storage/nfs_mounts/mount1

##ADD NFS
#litp inherit -p /deployments/d1/clusters/c1/nodes/n4/file_systems/nm1 -s /infrastructure/storage/nfs_mounts/nm1
#litp inherit -p /deployments/d1/clusters/c1/nodes/n4/file_systems/nm2 -s /infrastructure/storage/nfs_mounts/nm2
