...
# Based on RHEL6 expansion_2part_2node_3node_vxvm.sh
#  simplified:
#    only one disk in vg_root vol group
#    only creates one new SG
#    only updates one existing SG
#    no traffic3 network
#
# Usage:
#   RHEL7_expansion_2part_2node_3node_vxvm.sh <CLUSTER_SPEC_FILE>
#
...
# CREATE A PARALLEL SERVICE GROUP
litp create -p /software/images/image7 -t vm-image -o name="PL_SG_vm5" source_uri="http://${ms_ip}/images/vm_image_rhel7.qcow2"
litp create -p /software/services/vmservice7 -t vm-service -o service_name="CIvmserv7" image_name="PL_SG_vm5" cpus=4 ram=2500M internal_status_check=on cleanup_command="/usr/bin/systemctl stop CIvmserv7"
...
litp create -p /software/services/vmservice7/vm_yum_repos/os -t vm-yum-repo -o name=os base_url="http://${ms_ip}/7/os/x86_64"
litp create -p /software/services/vmservice7/vm_yum_repos/updates -t vm-yum-repo -o name=rhelPatches base_url="http://${net1vm_ip_ms}/7/updates/x86_64/Packages" # UPDATE
